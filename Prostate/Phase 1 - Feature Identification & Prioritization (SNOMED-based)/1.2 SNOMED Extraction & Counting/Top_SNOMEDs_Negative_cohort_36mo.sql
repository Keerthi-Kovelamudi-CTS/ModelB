...
/* === Final extract: LAST 36 MONTHS (recent medical history) === */
final_table AS (
  SELECT DISTINCT
    ncp.patient_guid,
    pp.sex,
  ...
  WHERE 
    -- 36-MONTH WINDOW (filter applied HERE, not in medical_history)
    mh.effective_date >= DATEADD(month, -36, DATE '2024-06-01')
