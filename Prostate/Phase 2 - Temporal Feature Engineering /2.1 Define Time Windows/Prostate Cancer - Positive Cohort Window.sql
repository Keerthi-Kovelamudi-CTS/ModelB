...
medical_history AS (
    ...
    WHERE effective_date IS NOT NULL
      ...
      AND effective_date < DATE '2025-11-01'
),

-- ============================================================================
-- PROSTATE CANCER PATIENTS WITH INDEX_DATE
-- INDEX_DATE = 12 months before diagnosis (prediction point)
-- ============================================================================
cancer_patients AS (
    SELECT
        mh.practice_id,
        ...
        -- NEW: INDEX_DATE = 12 months before diagnosis
        DATEADD(month, -12, MIN(mh.effective_date))           AS index_date
...
-- ============================================================================
-- 107 VALIDATED SNOMED CODES (CORRECTED - Verified against database)
-- ============================================================================
...
SELECT DISTINCT
    ...
    cp.index_date,                                              -- NEW: Index date for prediction
    DATEDIFF(year, pp.date_of_birth, cp. date_of_diagnosis) AS age_at_diagnosis,
    DATEDIFF(year, pp.date_of_birth, cp.index_date)        AS age_at_index,     -- NEW: Age at index date
    mh.effective_date                                      AS event_date,
    DATEDIFF(year, pp.date_of_birth, mh.effective_date)    AS event_age,
    
    -- NEW:  Months before INDEX_DATE (not diagnosis)
    DATEDIFF(month, mh.effective_date, cp.index_date)      AS months_before_index,
    
    -- NEW: Time windows relative to INDEX_DATE
    CASE 
        WHEN DATEDIFF(month, mh. effective_date, cp.index_date) BETWEEN 0 AND 11 THEN 'W1'   -- 0-12 months (MOST RECENT!)
        WHEN DATEDIFF(month, mh.effective_date, cp.index_date) BETWEEN 12 AND 23 THEN 'W2'  -- 12-24 months
        WHEN DATEDIFF(month, mh.effective_date, cp.index_date) BETWEEN 24 AND 35 THEN 'W3'  -- 24-36 months
        WHEN DATEDIFF(month, mh.effective_date, cp. index_date) BETWEEN 36 AND 47 THEN 'W4'  -- 36-48 months
        WHEN DATEDIFF(month, mh.effective_date, cp.index_date) BETWEEN 48 AND 60 THEN 'W5'  -- 48-60 months
        ELSE 'OUT_OF_RANGE'
    END AS time_window,
    ...
    -- NEW:  Target variable
    1                                                      AS cancer_in_next_12m   -- All cancer patients = 1
...
WHERE
    ...
    AND cp.index_date IS NOT NULL
    
    -- NEW: Get ALL events UP TO index_date (INCLUDES recent data!)
    AND mh.effective_date <= cp.index_date
    
    -- NEW: Go back 60 months from index_date (5 years of history)
    AND mh.effective_date >= DATEADD(month, -60, cp.index_date)
    
    -- Only your 108 validated SNOMED codes
