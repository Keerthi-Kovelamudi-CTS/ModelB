...
# 12mo uses bladder_feature_matrix_v2_cleaned.csv; 3mo/6mo use bladder_feature_matrix_{window}_cleaned.csv
if WINDOW == '12mo':
    INPUT_FILE = os.path.join(BLADDER_2, '2_Feature_Engineering', 'cleanupfeatures', WINDOW, 'bladder_feature_matrix_v2_cleaned.csv')
else:
...
def make_xgb():
    base = dict(eval_metric='auc', random_state=SEED, n_jobs=-1, use_label_encoder=False,
                tree_method='hist', early_stopping_rounds=EARLY_STOPPING_ROUNDS)
    if xgb_best_params:
        return xgb.XGBClassifier(**{**xgb_best_params, **base})
    return xgb.XGBClassifier(n_estimators=500, max_depth=6, learning_rate=0.05, subsample=0.8,
...
tree_names = [n for n in ['XGBoost','LightGBM','CatBoost'] if n in val_preds]
...
for name in val_preds:
    ...
    if valid:
        best = max(valid, key=lambda x: (x[2], x[1]))
...
if target_ops:
    print(f"\n--- Single best per model (by spec) ---")
    print(f"{'Model':<30s} {'Thresh':>7s} {'TestSens':>9s} {'TestSpec':>9s} {'PPV':>8s} {'NPV':>8s} {'TP':>5s} {'FN':>5s}")
    print("-" * 90)
    for name, r in sorted(target_ops.items(), key=lambda x: -x[1]['test_spec']):
        print(f"{name:<30s} {r['threshold']:>7.3f} {r['test_sens']:>8.2%} {r['test_spec']:>8.2%} "
              f"{r['test_ppv']:>7.2%} {r['test_npv']:>7.2%} {r['TP']:>5d} {r['FN']:>5d}")

    best_for_target = max(target_ops.keys(), key=lambda n: test_metrics.get(n, {}).get('auc', 0))
    rec_info = target_ops[best_for_target]
    print(f"\n★ Recommended (best AUC among those meeting target): {best_for_target} @ {rec_info['threshold']:.3f}")
...
deploy_meta = {
    ...
    'ensemble_weights': dict(zip(tree_names, best_w)) if USE_OPTUNA and len(tree_names)>=2 else None
}
