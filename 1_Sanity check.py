...
Outputs: sanity_check_results/ (timestamped log + summary CSV)
"""
...
# Paths: inputs from data/, results to sanity_check_results/
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = os.path.join(SCRIPT_DIR, 'data')
RESULTS_DIR = os.path.join(SCRIPT_DIR, 'sanity_check_results')
os.makedirs(RESULTS_DIR, exist_ok=True)

# Input files (meds may have .csv.csv extension)
...
def main():
    ...
    log_path = os.path.join(RESULTS_DIR, f'sanity_check_{timestamp}.log')
    tee = Tee(log_path)
    sys.stdout = tee
    try:
        run_checks(timestamp)
    finally:
        sys.stdout = tee.terminal
        tee.close()
    print(f"\nResults written to: {RESULTS_DIR}")
    print(f"Log file: sanity_check_{timestamp}.log")


def run_checks(timestamp):
    # Load
...
""")
    ...
    summary_path = os.path.join(RESULTS_DIR, f'sanity_check_summary_{timestamp}.csv')
