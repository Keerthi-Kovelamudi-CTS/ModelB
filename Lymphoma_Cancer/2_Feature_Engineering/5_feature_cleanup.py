# ═══════════════════════════════════════════════════════════════
# LYMPHOMA CANCER — FEATURE CLEANUP
# Step 5: Clean final feature matrices for modeling
# Input:  feature_matrix_final_*.csv (from Step 4 → 4b → 4c → 4d → 4e → 4f)
...
def cleanup_features(fm, window_name):
    ...
    if len(remaining) > 0:
        ...
        for col in remaining.index:
            ...
            # Days/gaps → -1 (no event happened)
                                          ...
                                          'LYM_DX_symptom_to_imaging_days']):
    ...
    # not from actual lab values. GFR is the worst offender.
    # ──────────────────────────────────────────────────────────
    print(f"\n── 5f2. REMOVE HEALTHCARE UTILIZATION CONFOUNDS ──")

    # Step 1: Remove all eGFR / glomerular filtration rate features (utilization artifact)
    egfr_cols = [c for c in fm.columns
                 if 'egfr' in c.lower() or 'glomerular' in c.lower()]
    if egfr_cols:
        print(f"  Removing {len(egfr_cols)} eGFR/GFR features (utilization artifact, not cancer signal):")
    ...
    # (Lymphoma pipeline likely has none — Leuk used LAB_RENAL_ category; harmless if 0 matches)
    ...
    if len(suspicious) > 0:
        print(f"  ⚠️ {len(suspicious)} features with 0.3-0.5 correlation (review):")
    ...
    print(f"\n  ◆ = lymphoma-specific (Step 4d) | ★ = advanced/mega (Step 4b/4c)")
    ...
    print(f"\n  Feature group breakdown:")
    prefixes = [
        ('LYM_LAB_', 'Lym lab thresholds'),
        ('LYM_DX_', 'Lym diagnostic pathway'),
        ('LYM_TX_', 'Lym treatment'),
        ('LYM_RF_', 'Lym risk factors'),
        ('LYM_', 'Lymphoma-specific'),
        ('OBS_', 'Observation'),
...
for window in ['3mo', '6mo', '12mo']:
    ...
    print(f"    Lymphoma-specific (◆): {len(lym_cols)}")
...
# ═══════════════════════════════════════════════════════════════
# LYMPHOMA-SPECIFIC SIGNAL CHECK
# ═══════════════════════════════════════════════════════════════

print(f"\n{'═'*70}")
print(f"  LYMPHOMA-SPECIFIC FEATURES — SIGNAL RANK")
print(f"{'═'*70}")

for window in ['3mo', '6mo', '12mo']:
    fm = cleaned_matrices[window]
    lym_cols = [c for c in fm.columns if c.startswith('LYM_') and c != 'LABEL']

    if lym_cols:
        lym_corr = fm[lym_cols].corrwith(fm['LABEL']).abs().sort_values(ascending=False)
        print(f"\n  {window} — Lymphoma features ranked:")
        for i, (col, corr) in enumerate(lym_corr.items()):
