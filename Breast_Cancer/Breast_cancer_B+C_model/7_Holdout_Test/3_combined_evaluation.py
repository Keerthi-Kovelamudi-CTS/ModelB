...
At two operating points:
  - Tier-0 threshold (primary, from saved config.json)
...
# 2_predict_unseen.py writes to 3_Modeling/results/2_predictions/{window}/
PREDICT_UNSEEN_RESULTS = BASE / "3_Modeling" / "results" / "2_predictions"
# This script's own outputs (combined_eval_*.csv etc.) go here:
...
def main():
    ...
    parser.add_argument("--window", required=True, choices=["1mo", "12mo"])
    ...
    log.info("=" * 70)
    log.info(f"  BREAST — COMBINED EVALUATION — {w}")
    log.info("=" * 70)

    # Paths
    train_pred_path   = TRAIN_RESULTS / w / f"predictions_{w}.csv"
    unseen_pred_path  = PREDICT_UNSEEN_RESULTS / w / f"predictions_unseen_{w}.csv"
    ...
    # Thresholds from saved config — handles both legacy single-seed (v2 / run 2)
    # and new multi-seed + per-band format (run 3+).
    try:
    ...
    # Tier-0 (default operating point): try new format first, fall back to legacy
    tier0_thr = cfg.get("global_threshold")
    if tier0_thr is None:
        tier0_thr = cfg.get("threshold")
    tier0_thr = float(tier0_thr)

    # Tier-90: new format saves per-band dict; legacy saves a scalar
    tier90_per_band = cfg.get("tier90_thresholds_per_band")
    if tier90_per_band:
        # Use median of band thresholds (typically all close anyway since one band dominates)
        vals = [float(v) for v in tier90_per_band.values() if v is not None and not (isinstance(v, float) and np.isnan(v))]
        tier90_thr = float(np.nanmedian(vals)) if vals else None
        if tier90_thr is not None:
            log.info(f"\n  Per-band tier90 thresholds: {tier90_per_band} → median={tier90_thr:.4f}")
    else:
    ...
    rethr_t92 = find_operating_point(yB, sB, min_sens=0.92)
    if rethr_t92[0] is not None:
        log.info(f"    Tier-92 (sens>=0.92, max spec):   thr={rethr_t92[0]:.4f} sens={rethr_t92[1]:.4f} spec={rethr_t92[2]:.4f}")
    else:
        log.info(f"    Tier-92 target (sens>=0.92): NOT ACHIEVABLE on combined")
    rethr_t95 = find_operating_point(yB, sB, min_sens=0.95)
    if rethr_t95[0] is not None:
        log.info(f"    Tier-95 (sens>=0.95, max spec):   thr={rethr_t95[0]:.4f} sens={rethr_t95[1]:.4f} spec={rethr_t95[2]:.4f}")
    else:
        log.info(f"    Tier-95 target (sens>=0.95): NOT ACHIEVABLE on combined")
    ...
    if rethr_t92[0] is not None:
        rows.append({
            "label": "RunB_Tier92_reoptimised", "threshold": rethr_t92[0],
            "sensitivity": rethr_t92[1], "specificity": rethr_t92[2],
            "n_total": n_combined, "n_positive": n_pos, "n_negative": n_neg,
        })
    if rethr_t95[0] is not None:
        rows.append({
            "label": "RunB_Tier95_reoptimised", "threshold": rethr_t95[0],
            "sensitivity": rethr_t95[1], "specificity": rethr_t95[2],
            "n_total": n_combined, "n_positive": n_pos, "n_negative": n_neg,
        })
    ...
    with open(out_txt, "w") as f:
        f.write(f"Breast combined evaluation — window={w}\n")
        f.write("=" * 70 + "\n")
