/*╔══════════════════════════════════════════════════════════════════╗
  ...
  ║    HAS_FAMILY_HX_BREAST_CA, HAS_FAMILY_HX_ANY_CA,             ║
  ║    HAS_BRCA_OR_HEREDITARY, HAS_PRIOR_CANCER_NON_BREAST,       ║
  ...
  ║       have ~1.7x baseline breast cancer risk).                ║
  ╚══════════════════════════════════════════════════════════════════╝*/
...
target_cancer_patients AS (
  ...
  WHERE TRUE  -- breast: cohort already sex-stratified upstream; keep both sexes
...
flag_codes AS (
  ...
  SELECT 'FAMILY_HX_BREAST_CA' AS flag_name, snomed_code FROM UNNEST([
    -- TODO: clinician to curate; the following are plausible placeholders.
    160338000,   -- Family history of malignant neoplasm of breast
    266702002    -- FH: Carcinoma of breast
  ]) AS snomed_code
  ...
  UNION ALL
  -- ── Prior cancer of any type (excluding the breast target itself) ──────
  ...
  SELECT 'PRIOR_CANCER_NON_BREAST', CAST(NULL AS INT64) FROM UNNEST([]) AS snomed_code
  ...
  SELECT 'BPH_EVER', snomed_code FROM UNNEST([
    266569009    -- Hyperplasia of breast (BPH)
  ]) AS snomed_code
...
-- Special case: "prior cancer (non-breast)" — driven by any cancer SNOMED
-- in the patient's history that is NOT a breast code AND before anchor-12mo.
prior_cancer_non_breast_hits AS (
  SELECT
    up.patient_guid,
    up.anchor_date,
    up.cancer_class,
    'PRIOR_CANCER_NON_BREAST' AS flag_name
  FROM unified_patients up
  JOIN medical_history mh USING (patient_guid)
  JOIN `prj-cts-ai-dev-sp.EMIS_MED_CODES.Dim_Cancer_Codes` dcc
    ON CAST(mh.snomed_c_t_concept_id AS STRING) = dcc.SNOMED_CODE
  ...
  WHERE LOWER(dcc.cancer_id) NOT LIKE '%disease%'
    AND LOWER(dcc.cancer_id) NOT LIKE p.target_cancer_pattern   -- exclude breast-itself
...
all_flag_hits AS (
  ...
  UNION ALL
  SELECT * FROM prior_cancer_non_breast_hits
)
...
SELECT
  ...
  IF(COUNTIF(h.flag_name = 'FAMILY_HX_BREAST_CA')     > 0, 1, 0) AS HAS_FAMILY_HX_BREAST_CA,
  ...
  IF(COUNTIF(h.flag_name = 'PRIOR_CANCER_NON_BREAST') > 0, 1, 0) AS HAS_PRIOR_CANCER_NON_BREAST,
