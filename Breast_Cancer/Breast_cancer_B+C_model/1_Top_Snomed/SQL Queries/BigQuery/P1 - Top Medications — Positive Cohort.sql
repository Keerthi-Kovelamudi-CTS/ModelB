...
-- ── LEAKAGE FIX (Task #21, 2026-05-31) ──
-- Breast-cancer-pathway SNOMED codes (treatment/surgery/RFC/chemo regimen).
-- Same list as the obs SQLs — used here to pull date_of_diagnosis EARLIER so the
-- lookback window starts before any post-diagnosis treatment marker appears.
...
-- Breast-cancer adjuvant therapy DMD med codes (post-diagnosis prescriptions).
breast_pathway_meds AS (
  -- REFINED 2026-06-01: tamoxifen + anastrozole RE-EXCLUDED.
  -- Although both have chemoprevention indications, UK practice is overwhelmingly
  -- adjuvant therapy (>95%). High OR (~30) in pre-fix Scores was driven by adjuvant
  -- signal, not chemoprevention. Keeping them excluded is the safer call.
...
feature_events AS (
  ...
  WHERE FLOOR(DATE_DIFF(cp.date_of_diagnosis, pp.date_of_birth, DAY) / 365.25) > 18
    ...
    -- LEAKAGE FIX (Task #21): exclude post-diagnosis adjuvant therapy meds
    AND NOT EXISTS (
