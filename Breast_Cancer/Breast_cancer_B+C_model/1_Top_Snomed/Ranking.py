...
CONFIG = {
    ...
    'p_value_test':        'fisher',  # 'fisher' (exact, recommended) or 'chi2'
...
def benjamini_hochberg(p_values):
    ...
    Less conservative than Bonferroni — controls expected false-discovery
    rate rather than family-wise error. Standard in biomarker discovery;
    lets through rare-but-strong signals (e.g. BRCA1 testing codes, low-
    prevalence comorbidities) that Bonferroni would crush with multiple-
    testing penalty.

    Returns array of adjusted p-values clipped to [0, 1]. NaN inputs → 1.0.
    """
...
def compute_statistical_ranking(pos_df, neg_df, feature_type='observation'):
    ...
    # like BRCA1 / hereditary cancer testing). Chi-squared is kept for the
    # chi2_score and backward compatibility. The chosen p-value (config.p_value_test)
    # is what feeds the correction + filter.
    ...
    # Bonferroni preserved as a column for transparency / sanity. FDR is the
    # default filter — controls expected-false-discovery rate and is less
    # conservative; rare-but-strong signals can pass.
