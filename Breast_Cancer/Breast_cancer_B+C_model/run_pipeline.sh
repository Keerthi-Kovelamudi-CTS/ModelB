#!/bin/bash
# Breast_Cancer_2.0 end-to-end pipeline (Approach B)
#
# Steps:
#   1. SQL extract  → data/raw/breast_1mo_raw.csv  (run separately in BQ Studio or via bq client)
#   2. Preprocess   → data/1mo/{breast_1mo_obs_dropped, breast_1mo_med_dropped}.csv
...
#   5. Holdout eval → 7_Holdout_Test/results/{combined_eval_1mo.txt, predictions_unseen_1mo.csv}
#
# Usage:
...
run_preprocess () {
  ...
  python3 0_preprocess.py \
    --raw     data/raw/breast_${WINDOW}_raw.csv \
    --out-dir data/${WINDOW} \
    --prefix  breast_${WINDOW}
}
...
run_eval () {
  ...
  if [ ! -s "data/fe_input/${WINDOW}/breast_${WINDOW}_obs_dropped.csv" ]; then
  ...
  python3 2_predict_unseen.py --window $WINDOW \
    --data "$ROOT/7_Holdout_Test/results/${WINDOW}/holdout_features_${WINDOW}.csv"
  cp "results/2_predictions/${WINDOW}/predictions_unseen_${WINDOW}.csv" \
     "$ROOT/7_Holdout_Test/results/predictions_unseen_${WINDOW}.csv"
