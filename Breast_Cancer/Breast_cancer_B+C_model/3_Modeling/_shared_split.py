...
def _extract_anchor_year_per_patient(cleanup_path: Path, raw_root: Optional[Path] = None) -> pd.Series:
    """Resolve patient_guid → anchor_year mapping by trying cleanup parquet → raw parquet → raw CSV.

    Returns a Series indexed by patient_guid (str), values are integer years.
    Patients without resolvable anchor year are dropped from the returned Series.
    """
    # Try cleanup parquet first
    df = pd.read_parquet(cleanup_path)
    if "PATIENT_GUID" not in df.columns:
        df = df.reset_index()
    df["PATIENT_GUID"] = df["PATIENT_GUID"].astype(str)

    for col in ("ANCHOR_YEAR", "anchor_year"):
        if col in df.columns:
            ay = (df[["PATIENT_GUID", col]].dropna()
                    .drop_duplicates("PATIENT_GUID")
                    .set_index("PATIENT_GUID")[col]
                    .astype(int))
            logger.info(f"    anchor_year resolved from cleanup parquet column '{col}': {len(ay):,} patients")
            return ay

    for col in ("ANCHOR_DATE", "anchor_date", "INDEX_DATE", "index_date"):
        if col in df.columns:
            ay = (df[["PATIENT_GUID", col]].dropna()
                    .drop_duplicates("PATIENT_GUID")
                    .assign(_year=lambda d: pd.to_datetime(d[col], errors="coerce").dt.year)
                    .dropna(subset=["_year"])
                    .set_index("PATIENT_GUID")["_year"]
                    .astype(int))
            logger.info(f"    anchor_year resolved from cleanup parquet date column '{col}': {len(ay):,} patients")
            return ay

    # Fallback: look in the raw preprocessed parquets.
    # EMIS preprocess renames ANCHOR_DATE → INDEX_DATE / ANCHOR_YEAR → no equivalent
    # (preprocess output has INDEX_DATE per event row). Try multiple column names.
    if raw_root is not None:
        for sub in ("breast_1mo_obs_dropped.parquet", "breast_12mo_obs_dropped.parquet",
                    "breast_1mo.parquet", "breast_12mo.parquet"):
            for p in raw_root.rglob(sub):
                # Read once to see what columns exist
                try:
                    head = pd.read_parquet(p).head(1)
                except Exception:
                    continue
                available = set(head.columns)
                date_col = None
                year_col = None
                for cand in ("ANCHOR_YEAR", "anchor_year"):
                    if cand in available: year_col = cand; break
                for cand in ("ANCHOR_DATE", "anchor_date", "INDEX_DATE", "index_date"):
                    if cand in available: date_col = cand; break
                if not year_col and not date_col:
                    continue
                cols_to_read = ["PATIENT_GUID"]
                if year_col: cols_to_read.append(year_col)
                if date_col: cols_to_read.append(date_col)
                raw = pd.read_parquet(p, columns=cols_to_read)
                raw["PATIENT_GUID"] = raw["PATIENT_GUID"].astype(str)
                if year_col:
                    ay = (raw.dropna(subset=[year_col])
                              .drop_duplicates("PATIENT_GUID")
                              .set_index("PATIENT_GUID")[year_col].astype(int))
                else:
                    ay = (raw.dropna(subset=[date_col])
                              .drop_duplicates("PATIENT_GUID")
                              .assign(_y=lambda d: pd.to_datetime(d[date_col], errors="coerce").dt.year)
                              .dropna(subset=["_y"])
                              .set_index("PATIENT_GUID")["_y"].astype(int))
                logger.info(f"    anchor_year resolved from raw parquet {p.name} via col='{year_col or date_col}': {len(ay):,} patients")
                return ay

    raise FileNotFoundError(
        f"Could not find ANCHOR_YEAR/ANCHOR_DATE/INDEX_DATE in {cleanup_path} or any raw parquet. "
...
def build_temporal_split(
    ...
    This is the methodologically-correct split for medical ML: train on past data,
    test on future data. Avoids leakage from coding-pattern drift, vaccine eras, etc.
    """
    ...
    # Drop patients without anchor_year — they can't be assigned a temporal split
    ...
    # Test = anchor_year >= temporal_test_year
    ...
    # Random stratified val split inside trainval
...
if __name__ == "__main__":
    ...
    p.add_argument("--mode", choices=["random", "temporal"], default="random",
                   help="random = stratified 75/10/15 (default, aligned to Truveta breast + Prostate_B+C — "
                        "balanced train/val/test, no temporal holdout); temporal = test >= --test-year, train+val before")
    ...
    p.add_argument("--raw-root", default=str(fe_cfg.BASE_PATH),
                   help="Where to look for raw preprocessed parquets if cleanup parquet lacks ANCHOR_YEAR")
    args = p.parse_args()
    ...
    if args.mode == "temporal":
        ...
        # Use first window's parquet as anchor source (anchor is patient-level, same across windows)
        any_window_path = next(iter(paths.values()))
        anchor_by_guid = _extract_anchor_year_per_patient(any_window_path, raw_root=Path(args.raw_root))
