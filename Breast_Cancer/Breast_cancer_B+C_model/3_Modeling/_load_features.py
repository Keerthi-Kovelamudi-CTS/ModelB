"""
_load_features.py — consume Alex's BQ-productionized feature tables.
...
=====
    ...
    from _load_features import load_alex_features

    # Whether the CSV is flat (today) or JSON-column (post-ADR), this works:
    df = load_alex_features("data/breast_features.csv")
...
JSON_COLUMN_NAME = "snomed_features"   # name of Alex's JSON column per his ADR
...
def load_alex_features(
    ...
    *,
    ...
    json_column: str = JSON_COLUMN_NAME,
    require_schema: bool = False,
) -> pd.DataFrame:
    """Load a feature table produced by Alex's pipeline.

    Handles both the legacy flat-column format AND the post-ADR JSON-column
    format transparently. Caller doesn't need to know which one they got.
    ...
    schema_path : str | Path, optional
        Path to Alex's `snomed_features_schema.json` sidecar (per ADR Condition #1).
        If provided, expanded JSON columns are cast to declared dtypes.
        If absent, dtypes are inferred (best-effort) — works but less reliable.
    expand_json : bool, default True
        If True and the JSON column exists, expand it to flat columns.
        Set False if you want to keep the JSON column intact (rare).
    json_column : str, default "snomed_features"
        Name of the JSON column to expand. Override if Alex changes the name.
    ...
    Examples
    --------
    >>> df = load_alex_features("breast_v4.csv")                   # auto-handles both formats
    >>> df = load_alex_features("breast_v4.parquet", "schema.json") # bulletproof dtypes
    >>> df = load_alex_features("breast_v4.csv", require_schema=True)  # production strict mode
    """
    path = Path(path)
    df = _read_table(path)
    n_cols_before = df.shape[1]

    if expand_json and json_column in df.columns:
        if schema_path is None and require_schema:
            raise ValueError(
                f"JSON column '{json_column}' present but no schema_path provided "
        ...
        df = _expand_json_column(df, json_column, schema)
        n_cols_after = df.shape[1]
        logger.info(
            f"  expanded '{json_column}' JSON: "
            f"{n_cols_before} → {n_cols_after} columns "
            f"({n_cols_after - n_cols_before + 1} features extracted)"
        )
    elif json_column in df.columns:
        logger.info(f"  '{json_column}' present but expand_json=False — left intact")
    else:
