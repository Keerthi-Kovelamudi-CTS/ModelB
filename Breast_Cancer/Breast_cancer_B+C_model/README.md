  ═════════════════════════════════════════════════════════════════════════
  BREAST_2.0_1to1 — 1:1 YEAR-STRATIFIED COHORT VARIANT  (SQL v4)
  ─────────────────────────────────────────────────────────────────────────
  Sibling of Breast_Cancer_2.0/. Cohort SQL is v4 — identical to v2
  (year-stratified, no age matching) but with the ratio set to 1:1 instead
  of 1:10. Train/val/test all reflect a balanced 1:1 cohort. Age is
  preserved as a predictive signal — the model is allowed to use age as a
  feature (cancer patients tend to be older).

  AUC stays comparable to the 1:9 baseline, but Sens/Spec/PPV numbers are
  NOT directly comparable since the test denominator differs.

  SQL v4 vs Breast_Cancer_2.0/SQL Queries/v2/*_v2.sql:
    line 48:  10  AS non_cancer_ratio  →  1  AS non_cancer_ratio
    (cohort sampling logic and structure are otherwise identical to v2)
  Re-run SQL → re-run FE → re-run modeling. Other code is unchanged.
  ═════════════════════════════════════════════════════════════════════════

---

## Current design (EMIS) — quick reference

> Aligned to the Truveta breast pipeline / `Prostate_B+C` (2026-06-04): the
> non-cancer anchor, ratio, split mode and balancing now match Truveta.

| Dimension | EMIS value |
|---|---|
| Source | EMIS GP records (UK — **no region/state filter**; states are Truveta-only) |
| Cancer def | female-only, SNOMED `%breast%` + cancer/carcinoma |
| Non-cancer anchor | **gold cancer-date pool** — each non-cancer patient is assigned a random date drawn from the cancer diagnosis-date distribution (`cancer_anchor_pool` + `MOD(ABS(FARM_FINGERPRINT(patient_guid)), N)+1`); guarantees anchor-YEAR alignment by construction. Year-stratified sampler caps per-year counts. |
| `non_cancer_ratio` | **5** (oversample buffer; balanced to 1:1 at split) |
| Split | **random** stratified 75/10/15 (`_shared_split.py` default), `build_shared_split(balance_to_1to1=True)` → train/val/test all ~1:1 |
| Approach B | yes — `0_preprocess.py` adds `__PLACEHOLDER__` rows for sparse patients |
| Sex matching | not needed — cohort is female-only |
| Feature engineering | category aggregations (6 aggs) + hand-engineered blocks (interactions built inline, in order) |
| Windows | 1mo + 12mo (dual-horizon) |

## Alignment with the Truveta breast pipeline (`/Truveta/Breast_Cancer`)

EMIS is now a mechanical twin of Truveta — same anchor, ratio, split, balancing,
Approach B, and FE design. The only differences are inherent to the data source:

| Dimension | EMIS | Truveta | Match? |
|---|---|---|---|
...
| Approach B / sparse patients | yes | yes | ✅ |
| 6-agg FE + hand layers + dual-horizon | yes | yes | ✅ |
| Region/state filter | n/a (UK) | 7 US states | data-source difference |
| Sex matching in sampler | n/a (female-only) | sex × year (cohort has both sexes) | data-source difference |

---

                                                                                                                                                                 
  [1] SQL EXTRACTION (SQL Queries/v4/{1mo,3mo,6mo,12mo}_v4.sql)                                                                                                  
      ─────────────────────────────────────────                                                                                                                  
      Cancer:     ALL breast cancer patients (no code filter)                                                                                                  
...
What you're describing                                                                                                                                         
  ...
  - Generic features now provide signal: "patient has clinical activity, just not breast-specific"                                                             
