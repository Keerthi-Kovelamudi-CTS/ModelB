...
def cleanup_features(fm, window_name, cfg):
    ...
    # ── PROTECTED COLUMNS — never drop these (needed for per-band calibration at inference time) ──
    PROTECTED_COLS = {'AGE_AT_INDEX', 'PATIENT_AGE', 'patient_age'}
    protected_snapshot = {}
    for col in PROTECTED_COLS:
        if col in fm.columns:
            # Cap outliers (we saw AGE_AT_INDEX reaching 166 from bad date parsing)
            v = pd.to_numeric(fm[col], errors='coerce')
            v = v.where((v >= 0) & (v <= 120), other=np.nan)
            v = v.fillna(v.median())  # impute outliers/missing with median
            protected_snapshot[col] = v.astype(float)
    ...
    # ── 5e. REMOVE NEAR-ZERO VARIANCE (>99.5% same value) ─────
    # Threshold relaxed 99 → 99.5: preserves rare-but-meaningful sparse features
    # without flooding model with pure noise columns.
    PRESERVE_SPARSE_PREFIXES = (
        # Breast-specific hand-engineered features (sparse but clinically meaningful):
        # postmenopausal weight gain, TAH+BSO, HbA1c diabetic flags, mental health
        # polypharmacy, etc.
        'BRCA_LAB_bmi_', 'BRCA_LAB_hba1c_',
        'BRCA_RF_has_tah', 'BRCA_RF_has_tah_bso', 'BRCA_RF_tah_no_bso',
        'BRCA_RF_early_menopause_proxy', 'BRCA_RF_postmenopausal_proxy',
        'BRCA_RF_metabolic_syndrome_proxy',
        'BRCA_RISK_high', 'BRCA_RISK_intermediate',
        'BRCA_TX_mh_polypharm', 'BRCA_TX_cv_polypharm_5plus',
        # Acceleration features (recent-velocity flags often sparse but signal-rich)
        'ACCEL_LIFE_BMI_', 'ACCEL_LAB_HBA1C_',
        'ACCEL_has_acute_onset', 'ACCEL_n_accelerating_categories',
    )
    logger.info(f"\n-- 5e. REMOVE NEAR-ZERO VARIANCE (>99.5% same value) --")
    numeric_cols = [c for c in fm.select_dtypes(include=[np.number]).columns if c != LABEL]
    nzv_cols = []
    for col in numeric_cols:
        if col.startswith(PRESERVE_SPARSE_PREFIXES):
            continue
    ...
    # ── 5i.5 RESTORE PROTECTED COLUMNS (per-band calibration uses these at inference) ──
    for col, vals in protected_snapshot.items():
        if col not in fm.columns:
            fm[col] = vals.reindex(fm.index).fillna(vals.median())
            logger.info(f"  Restored protected column '{col}' (was dropped during cleanup)")
...
def run_cleanup(cfg=None):
    ...
    for window in cfg.WINDOWS:
        ...
        path = cfg.FE_RESULTS / window / f'feature_matrix_final_{window}.parquet'
        if not path.exists():
            path = cfg.FE_RESULTS / window / f'feature_matrix_final_{window}.csv'
        if not path.exists():
            logger.warning(f"  Missing feature_matrix_final_{window} (.parquet or .csv)")
            continue
    ...
    for window in cfg.WINDOWS:
        ...
        out_path = cfg.CLEANUP_RESULTS / window / f'feature_matrix_clean_{window}.parquet'
        out_path.parent.mkdir(parents=True, exist_ok=True)
        cleaned.to_parquet(out_path, engine='pyarrow', index=True)
