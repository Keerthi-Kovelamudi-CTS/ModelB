...
def run_clean_data(cfg=None):
    ...
    for window in cfg.WINDOWS:
        ...
        # Prefer parquet, fall back to csv (read_table handles both)
        obs_path = data_dir / f'{cfg.DATA_PREFIX}_{window}_obs_dropped.parquet'
        med_path = data_dir / f'{cfg.DATA_PREFIX}_{window}_med_dropped.parquet'
        ...
        # Always write parquet (in-place if input was parquet, switch if was csv)
        out_obs = obs_path.with_suffix('.parquet')
        out_med = med_path.with_suffix('.parquet')
        write_table(obs, out_obs, index=False)
        write_table(med, out_med, index=False)
        logger.info(f"  Saved: {out_obs.name}, {out_med.name}")
