/*
 * Breast cancer prediction — 1mo window — Approach B (v4)
 *
 ...
 *
 * LOOKBACK WINDOW (1mo prediction, 12mo lookback = 1-month gap before anchor):
 *   - Events in [anchor - 12 months, anchor - 1 month]
 *
...
WITH params AS (
  SELECT
    ...
    1                 AS years_before,                -- 12mo lookback (short variant)
...
target_cancer_patients AS (
  ...
  FROM (
    ...
    JOIN patients pp ON pp.patient_guid = mh.patient_guid  -- both sexes retained (rare male breast cancer kept; SEX preserved as feature + fairness sub-group)
    CROSS JOIN params param
...
non_cancer_patients AS (
  ...
  WHERE NOT EXISTS (                                  -- both sexes retained (males kept for rare male breast cancer)
