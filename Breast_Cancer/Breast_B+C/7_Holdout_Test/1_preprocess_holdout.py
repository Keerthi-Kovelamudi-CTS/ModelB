...
ALIGNED to Breast_Cancer_2.0 pipeline:
...
Produces (per window):
  data/fe_input/{window}/breast_{window}_obs_dropped.csv
  data/fe_input/{window}/breast_{window}_med_dropped.csv

Keeps ALL non-training patients (both sexes) — including those with zero
breast-relevant codes. Patients with zero matches get a single
...
Filters:
  - (no sex filter — breast cohort includes both sexes, ~99% F, ~1% M)
...
WINDOWS                 = list(fe_config.WINDOWS)  # ['1mo','12mo']
...
def main():
    ...
    for w in WINDOWS:
        for et in ("obs", "med"):
            p = OUTPUT_DIR / w / f"breast_{w}_{et}_dropped.csv"
    ...
    all_candidates = {}      # PATIENT_GUID -> max observed AGE_AT_INDEX (after non-training filter)
    ...
    for ci, chunk in enumerate(reader):
        ...
        # Drop training patients first (no sex filter — keep both sexes)
        chunk = chunk.copy()
        if exclude:
        ...
        # Register every non-training patient (for placeholder coverage).
        ...
        if (ci + 1) % 20 == 0 or ci == 0:
            print(f"  chunk {ci+1}: total={total_rows:,} sex={sex_kept:,} "
                  ...
                  f"all_non_training={len(all_candidates):,} t={time.time()-t0:.0f}s")
