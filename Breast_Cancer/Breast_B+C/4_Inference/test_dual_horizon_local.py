...
def _get_patient_age(s):
    ...
    for col in ("AGE_AT_INDEX", "PATIENT_AGE", "patient_age", "age"):
        if col in s.index:
            v = s[col]
            try:
                fv = float(v)
    ...
    if not _AGE_FALLBACK_WARNED:
        logger.warning(
            "  ⚠ AGE_AT_INDEX/PATIENT_AGE not found in feature vector — falling back to 65 "
            "(per-band calibration will be inaccurate). Check that FE pipeline preserves the age column."
        )
...
def _xgb_native_shap(xgb_model, X):
    ...
    # Drop bias column → shape (n_rows, n_feats)
    return contribs[:, :-1]
...
def main():
    ...
    merged = pred_1mo.merge(
        pred_12mo, on="PATIENT_GUID", suffixes=("_1mo", "_12mo"), validate="one_to_one",
    )
    logger.info(f"Test patients (in both windows' test splits): {len(merged):,}")

    test_guids = merged["PATIENT_GUID"].tolist()
    ...
    fm_1mo_test  = (fm_1mo[fm_1mo["PATIENT_GUID"].isin(test_guids)]
                    .set_index("PATIENT_GUID").reindex(test_guids))
    fm_12mo_test = (fm_12mo[fm_12mo["PATIENT_GUID"].isin(test_guids)]
                    .set_index("PATIENT_GUID").reindex(test_guids))
