...
-- ── LEAKAGE FIX (Task #21, 2026-05-31): breast-cancer-pathway codes ──
-- Treatment / surgery / RFC / chemo codes that indicate a patient is on the breast
-- cancer pathway but aren't captured by Dim_Cancer_Codes (which holds diagnosis
-- codes only). Identified by auditing breast_combined_all.csv for codes with
-- huge OR + post-diagnosis clinical meaning.
breast_pathway_codes AS (
  SELECT snomed_code FROM UNNEST([
    -- RFC / admin / referral
    -- 874291000000100 (QCancer breast cancer risk) — RE-EXCLUDED 2026-06-01:
    -- score is calculated only when GP is already investigating breast cancer
    -- (decision-support trigger), so its presence ~= active suspicion. Soft leakage.
...
feature_events AS (
  ...
  WHERE FLOOR(DATE_DIFF(cp.date_of_diagnosis, pp.date_of_birth, DAY) / 365.25) > 18
    ...
    -- LEAKAGE FIX (Task #21): filter cancer-pathway codes
    AND NOT EXISTS (
