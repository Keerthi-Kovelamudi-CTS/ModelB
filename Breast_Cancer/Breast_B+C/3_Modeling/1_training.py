# ═══════════════════════════════════════════════════════════════
# BREAST CANCER — MODELING PIPELINE  (1:1 BALANCED-COHORT VARIANT)
...
def split_using_shared_assignment(X, y, shared_split, balance_train=False, balance_seed=42):
    ...
    if X.index.name != "PATIENT_GUID" and "PATIENT_GUID" not in X.columns:
        raise ValueError("split_using_shared_assignment requires PATIENT_GUID as index or column")
...
def run_modeling():
    ...
    for window in WINDOWS:
        ...
        def _resolve_age(X, n):
            for col in ('AGE_AT_INDEX', 'PATIENT_AGE', 'patient_age'):
                if col in X.columns:
