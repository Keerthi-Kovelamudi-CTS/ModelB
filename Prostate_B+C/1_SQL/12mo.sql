/*
 * Prostate cancer prediction — 12mo window — Approach B (v4)
 *
 ...
 *   - Cancer:     all prostate cancer patients diagnosed at any date (no anchor-window cutoff)
 *   - Non-cancer: ALL male patients without any cancer diagnosis, year-stratified
...
WITH params AS (
  SELECT
    ...
    DATE '2026-05-21' AS longterm_mh_end,
    5                 AS years_before,                -- 60mo lookback (5 years)
    12                 AS months_before,               -- 12mo gap before anchor (matches 12mo-prediction)
    5                 AS min_obs_events_per_patient,  -- symmetric (cancer + non-cancer): ≥10 events of any kind in 60mo (general data, not curated)
    1                 AS min_med_events_per_patient,  -- row-level filter on med output (not cohort gate)
    1                 AS non_cancer_ratio,            -- 1:1 cancer:non-cancer (balanced cohort)
...
patients AS (
  -- NOTE: Patient_Ethnicity table has been removed from EMIS_BULK_DATA_temp.
  -- Emitting NULL columns to preserve downstream schema. Ethnicity-derived
  -- features will be NULL in v3 (they were not in v2's top-25 features
  -- anyway, so comparison stays approximately fair).
  SELECT
    pa.patient_guid,
    pa.sex,
    pa.date_of_birth,
  ...
  FROM `cthesigns-platform-475414-b7.EMIS_BULK_DATA_PROCESSED.Admin_Patient` pa
...
target_cancer_patients AS (
  ...
  FROM (
    ...
    JOIN patients pp ON pp.patient_guid = mh.patient_guid AND pp.sex = 'M'
    CROSS JOIN params param
...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   All male patients without any cancer diagnosis, not in palliative care.
   ═══════════════════════════════════════════════════════════════════════════ */
...
non_cancer_patients AS (
  ...
  WHERE pp.sex = 'M'                                  -- prostate is male-only
    AND NOT EXISTS (
