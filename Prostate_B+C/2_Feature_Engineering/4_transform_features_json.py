...
# ----------------------------------------------------------------------
PROJECT_ID = 'cthesigns-platform-475414-b7'
...
# FE orchestration
# ======================================================================
def _preprocess_inmem(raw_df: pd.DataFrame, mapping_path: Path, time_window_mid: int = 30) -> pd.DataFrame:  # time_window_mid kept for back-compat; ignored in clinical-bin variant
    """
    ...
    # Bin-assignment dispatch:
    #   - Clinical/Simple variants:  assign_clinical_bin(months) — fixed boundaries from config
    #   - A/B variant:               assign_time_window(months, mid_months=time_window_mid)
    if hasattr(_prostate_preprocess, 'assign_clinical_bin'):
        df['TIME_WINDOW'] = _prostate_preprocess.assign_clinical_bin(months)
    else:
        df['TIME_WINDOW'] = _prostate_preprocess.assign_time_window(months, mid_months=time_window_mid)
...
def _run_fe_blocks(preprocessed_df: pd.DataFrame, window_label: str, cfg) -> pd.DataFrame:
    ...
    #   - Simple:    build_clinical_features(clin_df, cfg, window_name) — 3 args
    #   - All variants: build_medication_features(med_df, cfg)
    #   - All variants: build_interaction_features(fm, cfg)
    #   - All others:   (obs_df, med_df, fm, window_name, cfg)
...
def extract_prostate_features_json(
    ...
    # --- Preprocess (defensive anchor + categorize + bin-assignment) ---
    # Bin-assignment is variant-aware:
    #   - Clinical/Simple: fixed CLINICAL_BIN_BOUNDARIES from config
    #   - A/B:             per-window midpoint from cfg.TIME_WINDOW_MID_PER_WINDOW
    _ab_mid = getattr(config, 'TIME_WINDOW_MID_PER_WINDOW', {}).get(window, 30)
    preprocessed = _preprocess_inmem(raw, mapping_path, time_window_mid=_ab_mid)
    ...
    # If FE blocks emitted any of LABEL/AGE_AT_INDEX as features, drop them
    # from the cleaned matrix before the join so the demo values win
    # (the demo copies carry the canonical labels for the JSON-packed output).
    overlapping = [c for c in demo.columns if c in cleaned.columns]
    if overlapping:
        cleaned = cleaned.drop(columns=overlapping)
...
def main(sql_file: 'str | None' = None, window: 'str | None' = None,
         project_id: str = PROJECT_ID, upload_to_bq: bool = True) -> str:
    """End-to-end entry point: preprocessed CSV (or SQL) -> FE -> JSON pack -> BQ + local CSV.

    Window defaults to $WINDOW env var (set by run_pipeline.sh), else first entry in cfg.WINDOWS.
    Input defaults to data/{window}/prostate_{window}.csv (the output of 0_preprocess.py).
    """
    if window is None:
        window = os.environ.get('WINDOW', config.WINDOWS[0]).strip()
    if window not in config.WINDOWS:
        raise SystemExit(f"  ✗ window '{window}' not in cfg.WINDOWS={config.WINDOWS} — set WINDOW env var or pass --window")

    if sql_file is None:
        # Prefer the preprocessed CSV produced by 0_preprocess.py
        prep_csv = Path(__file__).resolve().parent / 'data' / window / f'prostate_{window}.csv'
        if prep_csv.exists():
            sql_file = str(prep_csv)
        else:
            # Fall back to raw CSV if preprocessing was skipped
            raw_csv = Path(__file__).resolve().parent / 'data' / 'raw' / f'prostate_{window}_raw.csv'
            if raw_csv.exists():
                sql_file = str(raw_csv)
            else:
                # Last resort: the per-window SQL file (will execute against BQ)
                sql_file = str(Path(__file__).resolve().parent.parent / '1_SQL' / f'{window}.sql')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sql',    type=str, default=None,
                        help='Path to input (.csv or .sql). Default: data/{window}/prostate_{window}.csv (output of 0_preprocess.py)')
    parser.add_argument('--window', type=str, default=None,
                        choices=config.WINDOWS,
                        help=f'Prediction window (default: $WINDOW env var or {config.WINDOWS[0]})')
