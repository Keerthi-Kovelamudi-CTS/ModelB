...
HORIZONS = ["12mo", "1mo"]      # always score BOTH horizons in one run
...
def set_horizon(horizon):
    """Point CONFIG at this horizon's Data/ + output/ dirs and attach a fresh per-horizon log file."""
    CONFIG['data_dir'] = os.path.join(_SCRIPT_DIR, 'Data', horizon)
    CONFIG['output_dir'] = os.path.join(_SCRIPT_DIR, 'output', horizon, 'Scores_lung')
...
if __name__ == '__main__':
    for horizon in HORIZONS:
        set_horizon(horizon)
        logger.info("#" * 80)
        logger.info(f"# HORIZON = {horizon}   (Data/{horizon}/ -> output/{horizon}/Scores_lung/)")
        logger.info("#" * 80)
        try:
            main()
