...
WEIGHT_TREND_FE   = _b("WEIGHT_TREND_FE", False)   # native weight-trend 40-window FE (bw_w); off by default
WEIGHT_TREND_CODE = _i("WEIGHT_TREND_CODE", 27113001)  # SNOMED body weight
...
# MEDICATION duration/drug-variety FE: per drug-class category, course-length (course_duration_in_days)
# stats + chronic-use fraction + distinct-drug count. Recovers treatment-intensity signal meds otherwise
# lose (they have no value_num). Dose/quantity are NOT in source data (only duration + drug_term pulled).
MED_FE         = _b("MED_FE", False)   # validated NEUTRAL (2026-06-26): survives stability but pruned as redundant with med occurrence features (0 gain); kept dormant, env-toggle MED_FE=1 to re-emit
MED_DUR_CAP_DAYS = _i("MED_DUR_CAP_DAYS", 365)   # cap course_duration_in_days outliers (raw max ~37k days)
