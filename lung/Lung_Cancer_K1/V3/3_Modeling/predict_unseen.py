...
class CancerPredictor:
    ...
    def predict(self, data_path):
        ...
        # ---- Deployment calibration: per-age-band King-Zeng to TRUE prevalence ----
        # The raw probability is calibrated to the balanced training base rate and is far too high at
        # the real ~0.18% prevalence. deploy_calibration applies a per-age-band log-odds offset so the
        # output is a trustworthy real-world probability (monotonic within band -> ranking unchanged).
        y_pred_proba_calibrated = None
        if y_pred_proba is not None:
            try:
                import sys as _sys
                _root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # pipeline root (Lung_Cancer_K1)
                if _root not in _sys.path:
                    _sys.path.insert(0, _root)
                import deploy_calibration as _dc
                _age = pd.to_numeric(df.get('patient_age'), errors='coerce') if 'patient_age' in df.columns else None
                if _age is not None and _age.notna().any():
                    _age = _age.fillna(_age.median()).values
                    y_pred_proba_calibrated = np.asarray(_dc.calibrate(y_pred_proba, _age), dtype=float)
                    print(f"✓ Applied per-age-band King-Zeng deployment calibration (true prevalence)")
                else:
                    print("[calib] patient_age unavailable - returning RAW probabilities (uncalibrated)")
            except Exception as _e:
                print(f"[calib] deployment calibration skipped: {_e}")
