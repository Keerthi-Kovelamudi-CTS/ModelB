...
### Step 3 — build the codelist  (both horizons)
```bash
python build_codelist.py
```
→ `build_codelist.py` assembles the **codelist** (`../2_FE/codelist/lung_codelist_{h}.csv`):
**curated** clinical codes (`curated_codes.json`) **∪** the **top-500** data-driven codes eligible by
`combined_rank` (gates: Bonferroni p<0.05 AND OR≥2 AND prevalence≥1%). Clinical priors + focused
discovery.
