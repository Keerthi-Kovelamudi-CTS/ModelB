...
import config as C
import splits
HORIZONS = list(C.HORIZONS)     # both horizons by default
if len(sys.argv) > 1:           # e.g. `python build_score_counts.py 12mo 5`  (horizon [years lookback])
...
def _train_guids(ev, horizon):
    """TRAIN patient guids (CLEANED) from the CANONICAL saved split — the single source of
    train/valid/test (splits.load_or_make / make_split.py). Code selection (counts + ML importance)
    uses TRAIN ONLY so the internal test never leaks into which codes get picked. The held-out 500/50k
    is excluded from the cohort SQL at source regardless."""
    pat = (ev.assign(_cg=_clean_guid(ev["patient_guid"]))[["_cg", "cancer_class"]]
             .drop_duplicates("_cg").sort_values("_cg").reset_index(drop=True))
    return splits.guids_for(splits.load_or_make(horizon, pat["_cg"], pat["cancer_class"]), "train")
...
def load_events(horizon):
    ...
    sql = re.sub(r"\d+(\s+AS years_before\b)", rf"{YEARS}\1", sql, count=1)       # scoring lookback (years)
    query = f"""
        SELECT patient_guid, cancer_class, event_type,
               CAST(snomed_c_t_concept_id AS STRING) AS snomed_c_t_concept_id, term,
               CAST(med_code_id AS STRING) AS med_code_id, drug_term
        FROM (
{sql}
        )
    """
