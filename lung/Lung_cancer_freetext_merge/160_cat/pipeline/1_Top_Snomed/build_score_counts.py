"""
NOTE: Phase-1 per-code scoring is SKIPPED when CATEGORIZED=True (run_v3 reads the hand-made category map
directly and never invokes this). It runs only for the per-SNOMED path (V3_snomed). Safe to run standalone,
but its outputs are unused by the categorized pipeline.
...
sys.path.insert(0, os.path.dirname(HERE))   # V3 root, for config + splits + events
import config as C
import splits
import events
import artifacts                          # atomic CSV writes (no partial score file on interruption)
HORIZONS = C.horizons_from_argv(sys.argv[1:])   # e.g. `build_score_counts.py 12mo 5`; hard-errors on a typo
# FE/scoring lookback in years: codes are scored on THIS lookback's TRAIN events, into Data/{h}/{YEARS}yr/
...
def load_events(horizon):
    """Cohort events for counting + matrices — from the shared GCS-cached events (pulled from BigQuery
    once per (horizon, lookback); see ../events.py). No per-step BigQuery re-scan."""
    ev = events.load_cohort_events(horizon, YEARS)
    print(f"  events: {len(ev):,} | {ev.patient_guid.nunique():,} patients "
          f"({ev[ev.cancer_class == 1].patient_guid.nunique():,} cancer / "
          f"{ev[ev.cancer_class == 0].patient_guid.nunique():,} non-cancer)")
    # LEAKAGE FIX: restrict scoring to the model's 80% TRAIN split via the SINGLE shared filter
    # (splits.filter_to_train) — so the codelist is selected WITHOUT the internal valid/test (and the
    # held-out 500/50k, already excluded at source). Every fitting consumer uses this same helper.
    ev = splits.filter_to_train(ev, horizon)
    print(f"  TRAIN-only (leak-safe) scoring: {ev.patient_guid.nunique():,} train patients "
          f"({ev[ev.cancer_class == 1].patient_guid.nunique():,} cancer / "
          f"{ev[ev.cancer_class == 0].patient_guid.nunique():,} non-cancer); internal valid/test + held-out excluded")
    return ev
...
def main():
    for horizon in HORIZONS:
        data_dir = os.path.join(HERE, "Data", horizon, f"{YEARS}yr")
        ...
        for cls, ftype, fname, code_col, term_col, out_code in COUNT_SPEC:
            ...
            artifacts.atomic_write(os.path.join(data_dir, fname), lambda t, _df=df: _df.to_csv(t, index=False))
        ...
        for ftype, name_col, fname in MATRIX_SPEC:
            ...
            artifacts.atomic_write(os.path.join(data_dir, fname), lambda t, _m=mat: _m.to_csv(t, index=False))
