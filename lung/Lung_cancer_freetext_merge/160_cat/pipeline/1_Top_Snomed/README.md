...
### Step 3 — build the codelist  (per horizon × lookback)
```bash
python build_codelist.py 12mo 5      # horizon [years lookback]
```
→ `build_codelist.py` assembles the **codelist** (`../2_FE/codelist/{Astra|Nova}_{yr}yr_OFF_codelist.csv`,
ranked, with `combined_rank/odds_ratio/p_value_bonferroni/prevalence_pos` columns):
the **top-500** data-driven codes eligible by `combined_rank` (gates: Bonferroni p<0.05 AND OR≥2 AND
prevalence≥1%). Each lookback re-discovers its codes on that lookback's own data.
