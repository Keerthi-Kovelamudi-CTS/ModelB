...
SHAP backend: TREE-EXACT where possible, in PROBABILITY space. A tree model -> TreeExplainer; a
soft-voting ensemble of trees -> the EXACT per-base-tree TreeExplainer weighted-averaged by the voting
weights — both explain ALL patients. (REQUIRES xgboost<3: shap-0.49's TreeExplainer can't parse
xgboost 3.x's vector base_score; see _member_tree_shap. requirements.txt pins it.) Members are explained
with model_output="probability" against a small background sample, so every member's SHAP is in the same
(probability) space and the voting-weighted sum exactly reconstructs the ensemble's predict_proba
(verified by additivity). NON-TREE models fall back automatically: a linear/logistic model (has `coef_`)
-> shap.LinearExplainer (fast, ALL patients, model-margin space); any other predict_proba model ->
shap.KernelExplainer (model-agnostic, probability space, slow — capped to `max_explained` patients).
...
def _shap_values(model, X, y=None, max_explained=None, seed=0):
    ...
    Tree models (or soft-voting ensembles of trees) use exact TreeExplainer with model_output="probability"
    against a small background sample (REQUIRES xgboost<3 — see _member_tree_shap); probability space makes
    the voting-weighted member sum a true decomposition of predict_proba. NON-TREE models fall back:
    a LINEAR/logistic model (has `coef_`) -> shap.LinearExplainer (fast, ALL patients, model-margin space);
    any other model with predict_proba -> shap.KernelExplainer (model-agnostic, probability space, SLOW —
    so capped to `max_explained` patients, default 300). (`max_explained` is ignored for tree models —
    they always explain ALL patients; for the KernelExplainer path it caps the sample. `seed` seeds the
    background sample.) Returns (sv, idx)."""
    ...
    # ---- non-tree fallback (baseline / linear / arbitrary models) ----
    if hasattr(model, "coef_"):                    # linear / logistic baseline -> fast, ALL patients (margin space)
        try:
            sv = _pos_class(shap.LinearExplainer(model, bg).shap_values(Xv))
            print(f"[explain] {name}: LinearExplainer SHAP, ALL {n:,} patients (model-margin space).")
            return sv, np.arange(n)
        except Exception as e:
            print(f"[explain] LinearExplainer failed ({type(e).__name__}: {e}); trying KernelExplainer.")
    # model-agnostic KernelExplainer (slow) -> cap the number explained (probability space)
    if not hasattr(model, "predict_proba"):
        raise RuntimeError(f"[explain] {name}: no tree route, not linear, and no predict_proba — cannot SHAP.")
    cap = int(max_explained) if max_explained else min(300, n)
    idx = np.arange(n) if cap >= n else np.sort(np.random.RandomState(seed).choice(n, cap, replace=False))
    kbg = shap.sample(Xv, min(50, n), random_state=seed)
    sv = _pos_class(shap.KernelExplainer(lambda d: model.predict_proba(d)[:, 1], kbg).shap_values(Xv[idx], silent=True))
    print(f"[explain] {name}: KernelExplainer (model-agnostic, probability space) on {len(idx):,}/{n:,} "
          f"patients (capped — slow/approximate; raise max_explained to cover more).")
    return sv, idx
