...
def _build_feature_schema_from_wide(
    ...
    """
    spine_candidates = [IDENTIFIER_COLUMN, *DEMOGRAPHIC_COLUMNS, TRAILING_COLUMN]
    ...
    features = {c: {'dtype': _infer_dtype(wide_df[c])} for c in feature_cols}
    return {
        'cancer':           cancer,
        'window':           window,
...
def write_to_bigQ(df: pd.DataFrame, table_suffix: str = '') -> str:
    ...
    # NOTE on `transformed_features` type: we use STRING here (matches lung's
    # table). The cross-team proposal recommends BigQuery native JSON for
    # *Alex's* table because consumers run aggregate / dot-path queries on it
    # (e.g. SELECT JSON_VALUE(features.bmi_slope) GROUP BY year). Our table is
    # the inference / serving table — the dominant query is single-patient
    # lookup (SELECT * WHERE patient_guid = ?) for which STRING and JSON
    # perform identically. Switch to JSON type if/when cross-row analytics
    # land on this table.
    schema = [
