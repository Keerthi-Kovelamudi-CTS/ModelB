...
if [ "$SKIP_BQ" = "1" ]; then
    banner "STEP 1/5: BQ extracts — SKIPPED"
else
    for window in "${WINDOWS[@]}"; do
        banner "STEP 1/5: BQ extract — $window"
...
if [ "$SKIP_FE" = "1" ]; then
    banner "STEP 2/5: FE pipeline — SKIPPED"
else
    for window in "${WINDOWS[@]}"; do
        banner "STEP 2/5: FE — $window"
        cd "$ROOT/2_Feature_Engineering"
        WINDOW=$window python3 0_preprocess.py
        WINDOW=$window python3 3_cleanup.py
        WINDOW=$window python3 4_transform_features_json.py
    done
fi

# ─── Step 3: Shared split ───
if [ "$SKIP_SPLIT" = "1" ]; then
    banner "STEP 3/5: Shared split — SKIPPED"
else
    banner "STEP 3/5: Build unified cohort + shared split"
    cd "$ROOT/3_Modeling"
...
if [ "$SKIP_TRAIN" = "1" ]; then
    banner "STEP 4/5: Training — SKIPPED"
else
    banner "STEP 4/5: Train 3 models (12mo, 1mo_5y, 1mo_12mo) on shared split"
    cd "$ROOT/3_Modeling"
...
if [ "$SKIP_COMPARE" = "1" ]; then
    banner "STEP 5/5: Comparison — SKIPPED"
else
    banner "STEP 5/5: Compare dual-horizon Setup A vs Setup B"
