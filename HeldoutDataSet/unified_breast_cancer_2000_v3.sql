/*
 * Unified cancer / non-cancer patient events for ML training — BREAST variant.
 *
 * This is a fixed-cohort variant of unified_cancer_noncancer42_v3.sql:
 *   - Cancer patients      = the 2,000 ids in  prj-cts-ai-dev-sp.EMIS_BULK_DATA_temp.breast_cancer_2000
 *   - Non-cancer patients  = the 50,000 ids in prj-cts-ai-dev-sp.EMIS_BULK_DATA_temp.no_cancer_breast_50000
 *
...
WITH params AS (
  SELECT
    ...
    '%breast%'          AS target_cancer_pattern        -- target cancer (must match breast_cancer_2000)
),
...
care_record_problem AS (
  ...
  QUALIFY ROW_NUMBER() OVER (
    PARTITION BY observation_guid ORDER BY SOURCE_DATE DESC
  ) = 1
),
...
/* Non-cancer anchor = one deterministically-sampled in-window observation event
   (identical to the logic that produced no_cancer_breast_50000). */
