/*
 * Unified cancer / non-cancer patient events for ML training — LUNG variant.
 *
 * This is a fixed-cohort variant of unified_cancer_noncancer42_v3.sql:
 *   - Cancer patients      = the 500 ids in  prj-cts-ai-dev-sp.EMIS_BULK_DATA_temp.lung_cancer_500
 *   - Non-cancer patients  = the 50,000 ids in prj-cts-ai-dev-sp.EMIS_BULK_DATA_temp.no_cancer_lung_50000
 *
...
WITH params AS (
  SELECT
    ...
    '%lung%'          AS target_cancer_pattern        -- target cancer (must match lung_cancer_500)
),
...
/* Non-cancer anchor = one deterministically-sampled in-window observation event
   (identical to the logic that produced no_cancer_lung_50000). */
