...
# Output: same prints to console and to results/sanity_check/sanity_check_leukaemia.log
SCRIPT_DIR = Path(__file__).resolve().parent
OUTPUT_DIR = SCRIPT_DIR / 'results' / 'sanity_check'
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
OUTPUT_FILE = OUTPUT_DIR / 'sanity_check_leukaemia.log'
...
files = {
    '3mo': {
        'clinical': f"{BASE_PATH}/3mo/FE_leuk_clinical_windowed_3m.csv",
        'med':      f"{BASE_PATH}/3mo/FE_leuk_med_windowed_3m.csv",
    },
    '6mo': {
        'clinical': f"{BASE_PATH}/6mo/FE_leuk_clinical_windowed_6m.csv",
        'med':      f"{BASE_PATH}/6mo/FE_leuk_med_windowed_6m.csv",
    },
    '12mo': {
        'clinical': f"{BASE_PATH}/12mo/FE_leuk_clinical_windowed_12m.csv",
        'med':      f"{BASE_PATH}/12mo/FE_leuk_med_windowed_12m.csv",
    },
}
...
try:
    print(f"Sanity check + Patient overlap check — Leukaemia pipeline")
    ...
    def patient_fixes(clin_df, med_df, window_name):
        ...
        if clin_df is not None:
            ...
            print(f"  (Both sexes expected for leukaemia — no sex filter applied)")
    ...
    for window in ['3mo', '6mo', '12mo']:
        ...
        if clin is not None:
            clin_path = f"{BASE_PATH}/{window}/FE_leuk_dropped_patients_clinical_windowed_{suffix}.csv"
            clin.to_csv(clin_path, index=False)
            print(f"\n  ✅ Saved: {clin_path} ({len(clin):,} rows)")

        if med is not None:
            med_path = f"{BASE_PATH}/{window}/FE_leuk_dropped_patients_med_windowed_{suffix}.csv"
