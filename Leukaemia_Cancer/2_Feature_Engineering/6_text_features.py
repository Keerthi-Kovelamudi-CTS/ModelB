# ═══════════════════════════════════════════════════════════════
# LEUKAEMIA CANCER — TEXT FEATURES (Keywords + TF-IDF + BERT)
...
CLINICAL_CATEGORIES = [
    ...
    'IMAGING', 'LYMPH_NODE_WORKUP', 'AUTOIMMUNE',
    'HAEM_INVESTIGATIONS', 'BLOOD_COUNT', 'KEY_BLOOD_TESTS'
]

TEXT_SEARCH_PATTERN = (
    'blood|haematol|lymph|spleen|marrow|blast|leuk|anaemi'
    '|platelet|white\\s*cell|bruise|petechiae'
)
...
def build_patient_documents(clin_df, window_name, truncate=False):
    ...
    # Also include haematology-related text from any category
    haem_text = has_text[has_text['CLEAN_TEXT'].str.contains(
        TEXT_SEARCH_PATTERN, na=False
    )]
    clinical_text = pd.concat([clinical_text, haem_text]).drop_duplicates()
...
def build_text_keywords(clin_df, window_name):
    ...
    # ══════════════════════════════════════════════════════════
    # BLOCK 1: HAEMATOLOGY / INVESTIGATION FINDINGS
    # ══════════════════════════════════════════════════════════
    print(f"  BLOCK 1: Haematology findings...")

    haem_text = has_text[has_text['CATEGORY'].isin([
        'HAEM_INVESTIGATIONS', 'BLOOD_COUNT', 'KEY_BLOOD_TESTS',
        'IMAGING', 'LYMPH_NODE_WORKUP', 'AUTOIMMUNE'
    ])].copy()
    # Also include any text mentioning blood/haem terms
    blood_text = has_text[has_text['CLEAN_TEXT'].str.contains(
        TEXT_SEARCH_PATTERN, na=False
    )]
    haem_text = pd.concat([haem_text, blood_text]).drop_duplicates()
    ...
    imaging_keywords = {
        ...
        'TEXT_IMG_blast_cells': r'blast\s*cell|immature\s*cell|promyelocyt|myelocyt',
    ...
    for feat_name, pattern in imaging_keywords.items():
        matches = haem_text[haem_text['CLEAN_TEXT'].str.contains(pattern, na=False, regex=True)]
        patient_match = matches.groupby('PATIENT_GUID').size() > 0
        tf[feat_name] = tf.index.map(patient_match).fillna(False).astype(int)

    # Has any haem text at all
    has_haem_text = haem_text.groupby('PATIENT_GUID').size() > 0
    tf['TEXT_IMG_has_report'] = tf.index.map(has_haem_text).fillna(False).astype(int)

    # Abnormal report (has text and NOT normal)
    haem_abnormal = haem_text[~haem_text['CLEAN_TEXT'].str.contains(
        r'normal|no\s*abnormal|unremark|no\s*significant|within\s*normal', na=False
    )]
    has_abnormal = haem_abnormal.groupby('PATIENT_GUID').size() > 0
    ...
    print(f"    Added {img_count} haematology finding features")
    ...
    clinical_categories = [
        'INFECTION', 'BLEEDING', 'B_SYMPTOMS', 'BONE_PAIN',
        'GI_SYMPTOMS', 'LYMPHADENOPATHY', 'SKIN_LYMPHOPROLIFERATIVE',
        'IMAGING', 'LYMPH_NODE_WORKUP'
    ]
    clinical_text = has_text[has_text['CATEGORY'].isin(clinical_categories)].copy()
    print(f"    Clinical text rows (non-lab): {len(clinical_text):,}")

    severity_keywords = {
        'TEXT_SEV_worsening': r'worsen|getting\s*worse|deteriorat|increas.{0,10}(pain|symptom|bleed|infect)',
        'TEXT_SEV_persistent': r'persistent\s*(pain|bleed|symptom|infect|fever|sweat|fatigue|bruise)',
        'TEXT_SEV_severe': r'severe\s*(pain|bleed|anaemi|infect|symptom|fatigue|bruise)',
        'TEXT_SEV_recurrent': r'recurrent\s*(pain|bleed|infection|episode|presentation|fever)',
        'TEXT_SEV_new': r'new\s*(symptom|lump|swelling|pain|bruise)|recently\s*(develop|start|notic)',
    }
    ...
    clinical_keywords = {
        'TEXT_CLIN_biopsy': r'biops|histolog|cytolog|bone\s*marrow|aspirat',
        'TEXT_CLIN_blood_film': r'blood\s*film|smear|peripheral\s*blood',
        'TEXT_CLIN_transfusion': r'transfus|blood\s*product|red\s*cell',
        'TEXT_CLIN_bruising': r'bruis|petechi|purpur|ecchymos',
        'TEXT_CLIN_night_sweats': r'night\s*sweat|drenching\s*sweat',
    ...
    # Abnormal blood finding + severity
    tf['TEXT_COMP_finding_with_severity'] = (
        ((tf.get('TEXT_IMG_abnormal_blood', 0) == 1) | (tf.get('TEXT_IMG_blast_cells', 0) == 1) |
...
if __name__ == '__main__':
    ...
    # ── Final summary ─────────────────────────────────────────
    print(f"\n{'═'*70}")
    print(f"  LEUKAEMIA TEXT FEATURE PIPELINE COMPLETE")
    print(f"{'═'*70}")
