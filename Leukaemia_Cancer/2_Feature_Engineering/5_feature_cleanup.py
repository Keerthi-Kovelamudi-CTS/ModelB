# ═══════════════════════════════════════════════════════════════
# LEUKAEMIA CANCER — FEATURE CLEANUP
...
def cleanup_features(fm, window_name):
    ...
    if len(remaining) > 0:
        ...
        for col in remaining.index:
            ...
            # Days/gaps → -1 (no event happened)
                                          ...
                                          'LEUK_DX_symptom_to_imaging_days']):
    ...
    print(f"\n  ◆ = leukaemia-specific (Step 4d) | ★ = advanced/mega (Step 4b/4c)")
    ...
    print(f"\n  Feature group breakdown:")
    prefixes = [
        ('LEUK_LAB_', 'Leuk lab thresholds'),
        ('LEUK_DX_', 'Leuk diagnostic pathway'),
        ('LEUK_TX_', 'Leuk treatment'),
        ('LEUK_RF_', 'Leuk risk factors'),
        ('LEUK_', 'Leukaemia-specific'),
        ('OBS_', 'Observation'),
...
for window in ['3mo', '6mo', '12mo']:
    ...
    print(f"    Leukaemia-specific (◆): {len(leuk_cols)}")
...
# ═══════════════════════════════════════════════════════════════
# LEUKAEMIA-SPECIFIC SIGNAL CHECK
# ═══════════════════════════════════════════════════════════════

print(f"\n{'═'*70}")
print(f"  LEUKAEMIA-SPECIFIC FEATURES — SIGNAL RANK")
print(f"{'═'*70}")

for window in ['3mo', '6mo', '12mo']:
    fm = cleaned_matrices[window]
    leuk_cols = [c for c in fm.columns if c.startswith('LEUK_') and c != 'LABEL']

    if leuk_cols:
        leuk_corr = fm[leuk_cols].corrwith(fm['LABEL']).abs().sort_values(ascending=False)
        print(f"\n  {window} — Leukaemia features ranked:")
        for i, (col, corr) in enumerate(leuk_corr.items()):
