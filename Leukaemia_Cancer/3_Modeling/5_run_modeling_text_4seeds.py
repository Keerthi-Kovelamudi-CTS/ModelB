# ═══════════════════════════════════════════════════════════════
# LEUKAEMIA CANCER — TEXT+EMBEDDINGS 4-SEED MODELING
...
VAL_RATIO = 0.10
TEST_RATIO = 0.15
SEEDS = [45]  # Best seed (highest avg AUC 0.872, most 80/70 hits)
...
def is_clinical_feature(col):
    ...
    # KEEP: Leukaemia-specific clinical features
    if col.startswith('LEUK_'):
        return True

    # KEEP: Leukaemia sub-prefix features
    if col.startswith('LEUK_LAB_'):
        return True
    if col.startswith('LEUK_DX_'):
        return True
    if col.startswith('LEUK_TX_'):
        return True
    if col.startswith('LEUK_RF_'):
        return True
