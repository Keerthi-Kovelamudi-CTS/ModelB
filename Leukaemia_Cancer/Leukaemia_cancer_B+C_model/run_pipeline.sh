#!/bin/bash
# Leukaemia_2.0_1to1 end-to-end pipeline (Approach B)
#
# Steps:
#   1. SQL extract  → data/raw/leukaemia_1mo_raw.csv  (run separately in BQ Studio or via bq client)
#   2. Preprocess   → data/1mo/{leukaemia_1mo_obs_dropped, leukaemia_1mo_med_dropped}.csv
...
run_preprocess () {
  ...
  for w in "${windows[@]}"; do
    ...
    python3 0_preprocess_to_fe.py \
      --raw     data/raw/leukaemia_${w}_raw.csv \
      --out-dir data/${w} \
      --prefix  leukaemia_${w}
  done
}
...
run_eval () {
  ...
  if [ ! -s "data/fe_input/${WINDOW}/leukaemia_${WINDOW}_obs_dropped.csv" ]; then
