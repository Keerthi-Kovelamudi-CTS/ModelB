# ═══════════════════════════════════════════════════════════════
# LEUKAEMIA — PREPROCESS RAW 300K DATA TO FE FORMAT
# Transforms raw Snowflake extract into the format expected by
# the Feature Engineering pipeline (4_Feature_engineering.py)
#
# Input:  300K_NonCancer_Patients.csv (raw SQL extract, ~144M rows)
# Output: data/{3mo,6mo,12mo}/FE_leuk_clinical_windowed_{suffix}.csv
#         data/{3mo,6mo,12mo}/FE_leuk_med_windowed_{suffix}.csv
#
# Alignment with training:
#   - Filters to ONLY the 228 approved codes (163 obs + 65 med)
#   - Maps CODE_ID → CATEGORY using exact training mapping
...
def window_and_save(all_data, all_patient_info):
    ...
    for window_name, window_def in WINDOW_DEFS.items():
        ...
        # Save
        clin_path = out_dir / f'FE_leuk_clinical_windowed_{suffix}.csv'
        med_path = out_dir / f'FE_leuk_med_windowed_{suffix}.csv'
...
if __name__ == '__main__':
    ...
    print(f"")
    print(f"  Next steps:")
    print(f"    1. Copy data/ to ../2_Feature_Engineering/data/")
    print(f"       (or update FE script BASE_PATH to point here)")
    print(f"    2. python3 ../2_Feature_Engineering/1_Sanitycheck+Patientoverlapcheck.py")
    print(f"    3. python3 ../2_Feature_Engineering/3_droppatients.py")
    print(f"    4. python3 ../2_Feature_Engineering/4_Feature_engineering.py")
    print(f"    5. python3 ../2_Feature_Engineering/5_feature_cleanup.py")
    print(f"    6. python3 ../2_Feature_Engineering/6_text_features.py")
    print(f"    7. python3 ../3_Modeling/predict_unseen.py --window 3mo \\")
    print(f"         --feature_matrix ../2_Feature_Engineering/results/Cleanup_Finalresults/3mo/feature_matrix_clean_3mo.csv")
    print(f"{'═'*60}")
