#!/bin/bash
# Melanoma_2.0_1to1 end-to-end pipeline (Approach B)
#
# Steps:
#   1. SQL extract  → data/raw/melanoma_1mo_raw.csv  (run separately in BQ Studio or via bq client)
#   2. Preprocess   → data/1mo/{melanoma_1mo_obs_dropped, melanoma_1mo_med_dropped}.csv
...
run_preprocess () {
  ...
  for w in "${windows[@]}"; do
    ...
    python3 0_preprocess_to_fe.py \
      --raw     data/raw/melanoma_${w}_raw.csv \
      --out-dir data/${w} \
      --prefix  melanoma_${w}
  done
}
...
run_eval () {
  ...
  if [ ! -s "data/fe_input/${WINDOW}/melanoma_${WINDOW}_obs_dropped.csv" ]; then
