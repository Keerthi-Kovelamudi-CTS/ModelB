# ═══════════════════════════════════════════════════════════════
# MELANOMA CANCER — TEXT+EMBEDDINGS 4-SEED MODELING
# Clinical features + text keywords + TF-IDF + BERT embeddings
# Full 4-seed evaluation with forced 80% sensitivity reporting
# ═══════════════════════════════════════════════════════════════
...
VAL_RATIO = 0.10
TEST_RATIO = 0.15
SEEDS = [45]  # 4 seeds for robust evaluation
USE_ALL_FEATURES = False  # Use feature selection (top N)
N_SELECT_FEATURES = 265  # 225 base + 10 text + 15 TF-IDF emb + 15 BERT emb
OPTUNA_TRIALS = 75

SCRIPT_DIR = Path(__file__).resolve().parent
BASE_PATH = SCRIPT_DIR.parent / '2_Feature_Engineering' / 'results' / 'Cleanup_Finalresults'
RESULTS_PATH = SCRIPT_DIR / 'results_text_emb_4seeds'
...
def is_clinical_feature(col):
    ...
    # KEEP: Melanoma-specific clinical features
    if col.startswith('MEL_'):
        return True

    # KEEP: Melanoma sub-prefix features
    if col.startswith('MEL_NICE_'):
        return True
    if col.startswith('MEL_PATH_'):
        return True
    if col.startswith('MEL_TX_'):
        return True
    if col.startswith('MEL_RF_'):
        return True
    if col.startswith('MEL_mimic_'):
        return True
    ...
    if col.startswith('INV_PATTERN_'):
        return True
    if col.startswith('INV_') and 'derm' in col.lower():
        return True
    if col.startswith('INV_') and 'hist' in col.lower():
        return True
    if col.startswith('INV_') and 'skin' in col.lower():
        return True
    ...
    # KEEP: Trajectory features for skin lesion/mole changes (clinically meaningful)
    if col.startswith('TRAJ_'):
        # Keep lesion, mole, and skin trajectories, increasing trend
        if 'lesion' in col.lower() or 'mole' in col.lower() or 'skin' in col.lower() or 'trend' in col.lower():
            return True
    ...
    # KEEP: Text and embedding features
    if col.startswith('TEXT_') or col.startswith('EMB_') or col.startswith('BERT_'):
        return True
...
# MERGE TEXT FEATURES
# ═══════════════════════════════════════════════════════════════
TEXT_PATH = SCRIPT_DIR.parent / '2_Feature_Engineering' / 'results' / 'Text_Features'

print(f"\n{'═'*70}")
print(f"  MERGING TEXT FEATURES")
print(f"{'═'*70}")

EMB_PATH = SCRIPT_DIR.parent / '2_Feature_Engineering' / 'results' / 'Text_Embeddings'
BERT_PATH = SCRIPT_DIR.parent / '2_Feature_Engineering' / 'results' / 'BERT_Embeddings'

for window in ['3mo', '6mo', '12mo']:
    before = clean_matrices[window].shape[1]

    # Merge text keyword features
    text_path = TEXT_PATH / window / f"text_features_{window}.csv"
    if text_path.exists():
        text_df = pd.read_csv(text_path, index_col=0)
        clean_matrices[window] = clean_matrices[window].join(text_df, how='left')

    # Merge TF-IDF embedding features
    emb_path = EMB_PATH / window / f"text_embeddings_{window}.csv"
    if emb_path.exists():
        emb_df = pd.read_csv(emb_path, index_col=0)
        clean_matrices[window] = clean_matrices[window].join(emb_df, how='left')

    # Merge BERT embedding features
    bert_path = BERT_PATH / window / f"bert_embeddings_{window}.csv"
    if bert_path.exists():
        bert_df = pd.read_csv(bert_path, index_col=0)
        clean_matrices[window] = clean_matrices[window].join(bert_df, how='left')

    clean_matrices[window] = clean_matrices[window].fillna(0)
    clean_matrices[window] = clean_matrices[window].loc[:, ~clean_matrices[window].columns.duplicated()]
    after = clean_matrices[window].shape[1]
    n_text = len([c for c in clean_matrices[window].columns if c.startswith('TEXT_')])
    n_emb = len([c for c in clean_matrices[window].columns if c.startswith('EMB_')])
    n_bert = len([c for c in clean_matrices[window].columns if c.startswith('BERT_')])
    print(f"  {window}: {before} → {after} features (+{n_text} text, +{n_emb} TF-IDF emb, +{n_bert} BERT emb)")
...
def select_features(X_train, y_train, n_top=150):
    ...
    # Force-include all text/embedding features, fill rest with top clinical
    ...
    # Rank clinical features by importance + correlation filter
    ...
    print(f"  Force-included {len(text_feats)} text features + {len(selected_clinical)} clinical = {len(final)} total")
...
for seed in SEEDS:
    for window in ['3mo', '6mo', '12mo']:
        ...
        for model_name, pred_test in predictions.items():
            ...
            # Forced operating points at specific sensitivity targets
            fpr_t, tpr_t, thresholds_t = roc_curve(y_val, pred_val)
            spec_t = 1 - fpr_t
            for target_sens, target_spec, label in [
                (0.80, 0.70, '80/70'),
                (0.80, 0.00, '80% Sens'),  # Just report spec at 80% sens
                (0.75, 0.65, '75/65'),
            ]:
                ...
                if valid.sum() > 0:
                    ...
                    if target_spec == 0.00:
                        # Just report what spec we get at this sens level
                        print(f"    → {label}: thresh={t:.4f} → TEST Sens={sens_f:.4f} Spec={spec_f:.4f}")
                        # Store for aggregation
                        result[f'spec_at_80sens'] = spec_f
                        result[f'sens_at_80target'] = sens_f
                    else:
                    ...
                    print(f"    → {label}: not achievable on val")
...
agg = results_df.groupby(['window', 'model']).agg({
    ...
    'meets_70_65': 'sum',
    'meets_70_60': 'sum',
    'spec_at_80sens': ['mean', 'std'],
    'sens_at_80target': ['mean', 'std'],
}).reset_index()
agg.columns = ['window', 'model', 'AUC_mean', 'AUC_std', 'Sens_mean', 'Sens_std', 'Spec_mean', 'Spec_std',
               ...
               'Spec80_mean', 'Spec80_std', 'Sens80_mean', 'Sens80_std']
agg['n_seeds'] = len(SEEDS)

print(f"\n  MEAN +/- SD (over {len(SEEDS)} seeds) — threshold from val, metrics on test:")
print(f"  {'Window':>6s} {'Model':>10s} {'AUC':>12s} {'Sens':>12s} {'Spec':>12s} {'Spec@80Sens':>14s} {'80/70':>8s} {'75/65':>8s} {'70/65':>8s}")
print(f"  {'─'*100}")
...
for _, r in agg.iterrows():
    ...
    spec80_str = f"{r['Spec80_mean']:.4f}±{r['Spec80_std']:.4f}" if r['Spec80_std'] == r['Spec80_std'] else f"{r['Spec80_mean']:.4f}"
    print(f"  {r['window']:>6s} {r['model']:>10s} {auc_str:>12s} {sens_str:>12s} {spec_str:>12s} {spec80_str:>14s} "
          f"{int(r['n_80_70'])}/{int(r['n_seeds']):<6d} {int(r['n_75_65'])}/{int(r['n_seeds']):<6d} {int(r['n_70_65'])}/{int(r['n_seeds'])}")
