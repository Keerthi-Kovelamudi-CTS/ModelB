...
prescribing_issuerecords_emis AS (
  SELECT
    ...
    SAFE_CAST(TRIM(code_id) AS INT64)                   AS code_id,
    SAFE_CAST(TRIM(course_duration_in_days) AS INT64)   AS course_duration_in_days,
    deleted,
    PARSE_DATE('%Y%m%d', REGEXP_EXTRACT(file_name, r'/([0-9]{8})/')) AS file_date
...
codes_emis AS (
  SELECT
    SAFE_CAST(TRIM(code_id) AS INT64)             AS code_id,
    SAFE_CAST(TRIM(dmd_product_code_id) AS INT64) AS dmd_product_code_id,
    TRIM(term)                                    AS term,
    source_practice_code,
    PARSE_DATE('%Y%m%d', REGEXP_EXTRACT(file_name, r'/([0-9]{8})/')) AS file_date
  FROM `prj-cts-ai-dev-sp.EMIS_BULK_DATA_PROCESSED.Coding_DrugCode`
  WHERE TRIM(dmd_product_code_id) IS NOT NULL
  QUALIFY ROW_NUMBER() OVER (
    PARTITION BY SAFE_CAST(TRIM(code_id) AS INT64), source_practice_code
    ORDER BY PARSE_DATE('%Y%m%d', REGEXP_EXTRACT(file_name, r'/([0-9]{8})/')) DESC
  ) = 1
),
