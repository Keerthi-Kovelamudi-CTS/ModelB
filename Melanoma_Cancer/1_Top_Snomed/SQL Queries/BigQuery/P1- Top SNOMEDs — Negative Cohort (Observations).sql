/*╔══════════════════════════════════════════════════════════════════╗
  ...
  ║  Window:  [-13 months, -1 month] before pseudo index date      ║
  ║                                                                 ║
  ║  Negative cohort:                                               ║
  ║    - Male + Female                                              ║
  ║    - Adults (>18)                                               ║
  ...
  ║    - Age-band + sex matched to melanoma cases (10:1)            ║
  ║    - Pseudo index = last record date                            ║
  ║    - Minimum 36 months GP history required                      ║
  ╚══════════════════════════════════════════════════════════════════╝*/

WITH params AS (
  SELECT
    DATE '1950-01-01'  AS mh_start,
    DATE '2026-03-11'  AS mh_end,
...
cancer_patients AS (
  ...
  INNER JOIN `prj-cts-ai-dev-sp.EMIS_MED_CODES.Dim_Cancer_Codes` AS dcc
    ON mh.snomed_c_t_concept_id = SAFE_CAST(dcc.SNOMED_CODE AS INT64)
  WHERE LOWER(dcc.cancer_id) NOT LIKE '%disease%'
    AND LOWER(dcc.cancer_id) LIKE '%melanoma%'
  GROUP BY 1
...
non_cancer_pool AS (
  ...
  WHERE FLOOR(DATE_DIFF(lr.last_record_date, p.date_of_birth, DAY) / 365.25) > 18
    ...
    AND DATE_SUB(lr.last_record_date, INTERVAL 36 MONTH) >= param.mh_start
...
-- TODO: Replace `EXPERIMENTS.MARTA_S.ETHNICITY_SNOMED_CODES` with the BigQuery
-- equivalent table (Snowflake-only reference). Keeping the name for now so the
-- intent is preserved; query will fail until this is wired up.
excluded_snomeds AS (
  SELECT SAFE_CAST(code AS INT64) AS snomed_code
  FROM `EXPERIMENTS.MARTA_S.ETHNICITY_SNOMED_CODES`
  UNION ALL
