<!-- ════════════════════════════════════════════════════════════════════ -->
<!-- MELANOMA — B+C MATURED MODEL  (generated 2026-06-04)            -->
<!-- ════════════════════════════════════════════════════════════════════ -->

# Melanoma — B+C Matured Model

Copy of `Melanoma_2.0_1to1` (originals untouched) with the **matured B+C cohort/split
design** finalized on breast and rolled out across all cancers. Source: **EMIS**.
Sex: **both sexes**. Dual/tri-horizon windows: **1mo + 12mo**.

## What's matured here (vs the parent `Melanoma_2.0_1to1`)
...
---
  ...
  ═════════════════════════════════════════════════════════════════════════
  MELANOMA_2.0_1to1 — 1:1 YEAR-STRATIFIED COHORT VARIANT  (SQL v4)
  ─────────────────────────────────────────────────────────────────────────
  ...
  TO BRING THIS ONLINE you need to provide:
    1. SQL Queries/v4/{1mo,3mo,6mo,12mo}_v4.sql
       - target_cancer_pattern    : '%melanoma%'
       - sex filter (in WHERE)    : both
       - non_cancer_ratio         : 1   (1:1 year-stratified)
       - cancer_snomed_codes      : melanoma-specific diagnosis codes
    2. codelists/code_category_mapping_v2.json   (melanoma curated codes)
    3. codelists/melanoma_curated_codes_v2.tsv
    4. 2_Feature_Engineering/config.py
       - replace prostate OBS/MED/LAB/CLUSTER/INTERACTION configs with
         melanoma-relevant ones (markers in file).
    5. 2_Feature_Engineering/4_cancer_features.py
       - currently PSA-specific; rewrite with melanoma markers
         (e.g., melanoma-specific labs, imaging, symptoms).
  ...
  [1] SQL EXTRACTION (SQL Queries/v4/{1mo,3mo,6mo,12mo}_v4.sql)  ← TODO: melanoma cohort─────────────────────────────────────────                                                                                                                  
