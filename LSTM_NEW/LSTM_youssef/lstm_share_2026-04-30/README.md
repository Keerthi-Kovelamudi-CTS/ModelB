# LSTM Pipeline k1 — Lung & Prostate Cancer (2026-04-30 retrain)

**This is the regenerated version of the 2026-04-28 share.** Includes encoder + threshold artifacts at training time so re-runs from saved model are bit-for-bit reproducible.

> 📁 The original 2026-04-28 share is preserved at `lstm_share_2026-04-28/` for historical reference (its `.keras` files don't have matching artifacts — bug #9, fixed in this retrain).

## Final results (2026-04-30 retrain)

**Configuration (locked):** `v45_balanced_min3` SQL + `max_snomed=50` + dual-branch rating bug fix + `min_code_frequency=5` + `min_snomed_seq=3` (+ med branch for lung only). Single seed=42.

| Cancer Type | Model | Type | Sensitivity | Specificity | Threshold | Patients | Pos | Neg | train/val/test | Accuracy | F1 | ROC-AUC | TP / TN / FP / FN |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Lung | pipeline_k1 | LSTM-dual (snomed+med) | **92.23%** | **71.65%** | 0.5472 | 3,863 | 1,926 | 1,937 | 3,091 / 386 / 387 | 81.91% | 83.57% | **88.20%** | 178 / 139 / 55 / 15 |
| Prostate | pipeline_k1 | LSTM-snomed | 94.21% | 69.41% | 0.4874 | 35,948 | 17,974 | 17,974 | 28,758 / 3,595 / 3,595 | 81.81% | 83.81% | **88.58%** | 1,693 / 1,248 / 550 / 104 |

**Comparison to 2026-04-28 original:**

| Cancer | Date | AUC | Sens | Spec | Reproducible from saved files? |
|---|---|---|---|---|---|
| Lung | 2026-04-28 (single seed) | 0.904 | 91.19% | 76.29% | ❌ no encoder/threshold saved |
| **Lung** | **2026-04-30 (single seed, this retrain)** | **0.882** | **92.23%** | **71.65%** | **✅ yes — _artifacts.joblib** |
| Prostate | 2026-04-28 (seed 42, best of 3) | 0.936 | 93.60% | 82.37% | ❌ no encoder/threshold saved |
| **Prostate** | **2026-04-30 (single seed)** | **0.886** | **94.21%** | **69.41%** | **✅ yes — _artifacts.joblib** |

**Why metrics differ slightly from 2026-04-28:** between the two runs, BQ data accumulated more patients/codes; encoder rebuilds with slight differences; fresh training from random init lands on a slightly different optimum. Original prostate run used **3 seeds and picked the best (seed 42 was lucky)** — this retrain uses single seed only. Multi-seed retrain would likely recover ~0.91-0.93 AUC.
