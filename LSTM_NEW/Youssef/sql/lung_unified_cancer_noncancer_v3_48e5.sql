 ...
 * NOTE on ETHNICITY
 *
 * patient_ethnicity is computed per-patient with the `patient_ethnicity_lookup` CTE
 * (latest ethnicity-recording SNOMED in the patient's medical_history).
 * The result is joined once per patient_guid into BOTH
 * observation_events_raw and medication_events_raw, so every row of a
 * patient carries the same ethnicity value, and medication rows are no
 * longer NULL. Computation is identical for cancer and non-cancer
 * patients (no class-conditional logic). 
 */
...
WITH params AS (
  SELECT
    ...
    -- 10                AS years_before,
    12                AS months_before,
    5                 AS min_obs_events_per_patient,  -- minimum observation events required per patient
...
medical_history AS (
  ...
  WHERE co.effective_date IS NOT NULL
   ...
   QUALIFY ROW_NUMBER() OVER (
    PARTITION BY co.observation_guid ORDER BY co.source_date DESC
  ) = 1
),
...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════════════════ */
/* ═══════════════════════════════════════════════════════════════════════════
   per-patient ethnicity lookup
   ─────────────────────────────────────────────────────────────────────────
   For each patient, pick a single ethnicity from their full medical history
   by selecting the LATEST observation whose own SNOMED code is itself an
   ethnicity-recording code (joined via Ethnicity_Snowmed_Codes). Patients
   without any such recording fall through as NULL — pre_process maps NULL
   to 'unknown' downstream.

   Symmetric for both classes: cancer and non-cancer patients are looked up
   identically (no class-conditional logic). The result is then JOINed once
   per patient_guid into observation_events_raw and medication_events_raw,
   so EVERY row of a patient carries the same ethnicity value.
   ═══════════════════════════════════════════════════════════════════════════ */

patient_ethnicity_lookup AS (
  -- also pulls rc.Label_6 and rc.Label_16 from the SAME chosen row,
  -- so all three columns (ethnicity + the two grouping labels) are
  -- consistent with each other for a given patient.
  SELECT
    patient_guid,
    ethnicity AS patient_ethnicity,
    label_6   AS ethnicity_grouping_label_6,
    label_16  AS ethnicity_grouping_label_16
  FROM (
    SELECT
      mh.patient_guid,
      CAST(rc.ethnicity AS STRING) AS ethnicity,
      CAST(rc.Label_6   AS STRING) AS label_6,
      CAST(rc.Label_16  AS STRING) AS label_16,
      ROW_NUMBER() OVER (
        PARTITION BY mh.patient_guid
        ORDER BY mh.effective_date DESC,
                 mh.snomed_c_t_concept_id DESC
      ) AS rn
    FROM medical_history mh
    JOIN `cthesigns-platform-475414-b7.EMIS_MED_CODES.Ethnicity_Snowmed_Codes` rc
      ON CAST(mh.snomed_c_t_concept_id AS STRING) = rc.code
    WHERE mh.effective_date IS NOT NULL
      AND rc.ethnicity IS NOT NULL
  )
  WHERE rn = 1
),
...
observation_events_raw AS (
  SELECT DISTINCT
    ...
    pe.patient_ethnicity                                      AS patient_ethnicity,             -- v47e: per-patient lookup, same value across all rows
    pe.ethnicity_grouping_label_6                             AS ethnicity_grouping_label_6,    -- v48e: 6-group ethnicity label from same lookup row
    pe.ethnicity_grouping_label_16                            AS ethnicity_grouping_label_16,   -- v48e: 16-group ethnicity label from same lookup row
  ...
  LEFT JOIN patient_ethnicity_lookup pe
    ON pe.patient_guid = up.patient_guid  -- v47e: per-patient ethnicity (was: per-event SNOMED-code lookup)
  CROSS JOIN params param
  WHERE
...
medication_events_raw AS (
  SELECT DISTINCT
    ...
    DATE_DIFF(param.longterm_mh_end,
              PARSE_DATE('%Y-%m-%d', pp.date_of_birth), YEAR) AS patient_age,
    pe.patient_ethnicity                                      AS patient_ethnicity,             -- v47e: per-patient lookup (was: hardcoded NULL on med rows)
    pe.ethnicity_grouping_label_6                             AS ethnicity_grouping_label_6,    -- v48e: 6-group ethnicity label from same lookup row
    pe.ethnicity_grouping_label_16                            AS ethnicity_grouping_label_16,   -- v48e: 16-group ethnicity label from same lookup row
  ...
  JOIN patients pp
    ON pp.patient_guid = up.patient_guid
  LEFT JOIN patient_ethnicity_lookup pe
    ON pe.patient_guid = up.patient_guid  -- v47e: per-patient ethnicity (was: NULL on med rows)
  CROSS JOIN params param
  WHERE
