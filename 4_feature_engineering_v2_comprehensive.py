...
═══════════════════════════════════════════════════════════════
  ...
  YOUR ORIGINAL v2 (COMPLETE) + ALL NEW v4 ADDITIONS
  
  Input:  FE_bladder_clinical_windowed.csv
          FE_bladder_med_windowed.csv
  Output: bladder_feature_matrix_v4.csv
...
clinical = pd.read_csv('FE_bladder_clinical_windowed.csv')
meds = pd.read_csv('FE_bladder_med_windowed.csv')
...
total_all = get_col('TEMP_ALL_WA',0)+get_col('TEMP_ALL_WB',0# (continuing from RATIO features)
)+get_col('TEMP_ALL_WCOMORB',0)+get_col('TEMP_ALL_WRF',0)
...
output_path = 'bladder_feature_matrix_v4.csv'
fm.reset_index().to_csv(output_path, index=False)
print(f"Feature matrix saved to: {output_path}")
print(f"  Shape: {fm.shape}")

meta.reset_index().to_csv('bladder_feature_meta_v4.csv', index=False)
print(f"Meta saved to: bladder_feature_meta_v4.csv")

with open('bladder_feature_list_v4.txt', 'w') as f:
...
print(f"Feature list saved to: bladder_feature_list_v4.txt")
