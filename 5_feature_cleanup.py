...
Run AFTER feature_engineering_v2
Input:  FE_Results/bladder_feature_matrix_v2.csv
Output: cleanupfeatures/ (cleaned matrix + feature list)
"""
...
# Paths: input from FE_Results/, output to cleanupfeatures/
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
INPUT_DIR = os.path.join(SCRIPT_DIR, 'FE_Results')
OUTPUT_DIR = os.path.join(SCRIPT_DIR, 'cleanupfeatures')
os.makedirs(OUTPUT_DIR, exist_ok=True)
INPUT_FILE = os.path.join(INPUT_DIR, 'bladder_feature_matrix_v2.csv')
...
output_path = os.path.join(OUTPUT_DIR, 'bladder_feature_matrix_v2_cleaned.csv')
cleaned.reset_index().to_csv(output_path, index=False)
print(f"\nSaved to: {output_path}")
print(f"  Shape: {cleaned.shape}")

# Feature list
feature_list_path = os.path.join(OUTPUT_DIR, 'bladder_feature_list_v2_cleaned.txt')
