...
def cleanup_features(fm, window_name, cfg):
    ...
    # (e.g. Free PSA, rare comorbidities) without flooding model with pure noise columns.
    PRESERVE_SPARSE_PREFIXES = (
        # Free PSA / Free:total ratio (~0.3% coverage, strong signal when present)
        'LAB_PSA_FREE_', 'LAB_PSA_RATIO_', 'OBS_PSA_FREE_', 'OBS_PSA_RATIO_',
        'CLUSTER_PSA_FREE_', 'CLUSTER_PSA_RATIO_', 'RECUR_PSA_FREE_', 'RECUR_PSA_RATIO_',
        'DECAY_PSA_FREE_', 'DECAY_PSA_RATIO_',
        # New BLOCK 16: PSA jump / lab co-elevation / symptom burst (binary flags often sparse)
        'PROST_PSA_jump_over_', 'PROST_PSA_relative_jump_',
        'PROST_LABPAIR_psa_alp_', 'PROST_LABPAIR_psa_high_anaemia',
        'PROST_SYMPTOM_burst_',
        # Acceleration block: recent-velocity high/very-high binary flags
        'ACCEL_PSA_recent_velocity_high', 'ACCEL_PSA_recent_velocity_very_high',
        'ACCEL_has_acute_onset',
    )
    logger.info(f"\n-- 5e. REMOVE NEAR-ZERO VARIANCE (>99.5% same value) --")
    numeric_cols = [c for c in fm.select_dtypes(include=[np.number]).columns if c != LABEL]
    nzv_cols = []
