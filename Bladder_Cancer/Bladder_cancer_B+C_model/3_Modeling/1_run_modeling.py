# ═══════════════════════════════════════════════════════════════
# BLADDER CANCER — MODELING PIPELINE  (1:1 BALANCED-COHORT VARIANT)
# Clinical + Text + TF-IDF + BERT embeddings
# Multi-seed evaluation with tiered sensitivity/specificity
#
# Cohort source: 1:1 bladder cohort via SQL Queries/v4/*_v4.sql (non_cancer_ratio = 1).
...
SPLIT_MANIFEST_PATH = _os.environ.get('SPLIT_MANIFEST', '')  # frozen-split manifest (B1 ratio); '' = normal split
...
def _split_from_manifest(X, y, window, manifest_path):
    """Frozen split for the B1 ratio experiment: partition by pre-defined
    train/val/test PATIENT_GUID lists (identical patients across the 1:1 and 1:10
    models; NO random split, NO 1:1 balance)."""
    import json
    man = json.load(open(manifest_path))[window]
    idx = X.index.astype(str)
    ya = np.asarray(y)
    def pick(guids):
        m = idx.isin(set(map(str, guids))).to_numpy()
        return X[m], ya[m]
    Xtr, ytr = pick(man['train']); Xv, yv = pick(man['val']); Xte, yte = pick(man['test'])
    logger.info(f"  SPLIT_MANIFEST {window}: train={len(Xtr)} (pos {int(ytr.sum())}) | "
                f"val={len(Xv)} | test={len(Xte)} (pos {int(yte.sum())}) — frozen, no balance")
...
def train_val_test_split(X, y, seed):
    ...
    discarded (the 5x buffer absorbs the trim). Mirrors the Truveta/breast
    build_shared_split(balance_to_1to1=True) behaviour.
    """
    y_arr = np.asarray(y)
...
def run_modeling():
    ...
    for window in WINDOWS:
        ...
        if SPLIT_MANIFEST_PATH:
            X_train, X_val, X_test, y_train, y_val, y_test = _split_from_manifest(X, y, window, SPLIT_MANIFEST_PATH)
        else:
