# ═══════════════════════════════════════════════════════════════
# BLADDER CANCER — COMBINED EVALUATION
# Run A: Non-cancer only → specificity, FPR, risk stratification
# Run B: Held-out test cancer + non-cancer → full metrics (AUC, Sens, Spec)
#
# Usage:
...
#
# Training predictions are at: results/{window}/test_predictions.csv
# with column ACTUAL_LABEL (not LABEL).
# ═══════════════════════════════════════════════════════════════
...
def load_config(window):
    """Load thresholds and config from model_summary_v4.json."""
    config_path = SCRIPT_DIR / 'results' / window / 'model_summary_v4.json'
...
def compute_full_report(predictions, config, run_name):
    ...
    # Get target threshold from config
...
def main():
    parser = argparse.ArgumentParser(description='Bladder Cancer — Combined evaluation: non-cancer only + cancer+non-cancer')
    parser.add_argument('--window', required=True, choices=['3mo', '6mo', '12mo'])
    parser.add_argument('--unseen_predictions', required=True,
                        help='Predictions CSV from predict_unseen.py (non-cancer patients)')
    parser.add_argument('--output', default='combined_evaluation.txt')
    args = parser.parse_args()

    print(f"{'='*60}")
    print(f"  BLADDER CANCER — COMBINED EVALUATION")
    print(f"  Window: {args.window}")
    print(f"{'='*60}")

    config = load_config(args.window)
    print(f"  Config from: results/{args.window}/model_summary_v4.json")
    if 'target' in config:
        print(f"  Target model: {config['target'].get('model', 'unknown')}")
        print(f"  Target threshold: {config['target'].get('threshold', 0.5)}")

    # Load unseen predictions (non-cancer)
    ...
    # Bladder uses ACTUAL_LABEL and stores at results/{window}/test_predictions.csv
    training_pred_path = SCRIPT_DIR / 'results' / args.window / 'test_predictions.csv'
    print(f"\n  Loading training test predictions: {training_pred_path}")
    if not training_pred_path.exists():
        print(f"  ERROR: Training predictions not found at {training_pred_path}")
        sys.exit(1)

    training_pred = pd.read_csv(training_pred_path)

    # Normalize column name: ACTUAL_LABEL → LABEL for consistency
    if 'ACTUAL_LABEL' in training_pred.columns and 'LABEL' not in training_pred.columns:
        training_pred = training_pred.rename(columns={'ACTUAL_LABEL': 'LABEL'})
    ...
    prob_cols_unseen = [c for c in unseen.columns if c.endswith('_prob')]
    prob_cols_training = [c for c in training_pred.columns if c.endswith('_prob')]
    common_prob_cols = sorted(set(prob_cols_unseen) & set(prob_cols_training))

    if not common_prob_cols:
        print(f"\n  WARNING: No common probability columns between unseen and training predictions!")
        print(f"  Unseen prob cols: {prob_cols_unseen}")
        print(f"  Training prob cols: {prob_cols_training}")
        # Use unseen columns as-is for Run A, and training columns for the cancer portion
        common_prob_cols = prob_cols_unseen
        print(f"  Using unseen columns for evaluation: {len(common_prob_cols)}")

    common_cols = ['PATIENT_GUID', 'LABEL'] + common_prob_cols
    flag_cols = [c for c in unseen.columns if c.endswith('_flag')]
    common_flag_in_training = [c for c in flag_cols if c in training_pred.columns]
    common_cols += common_flag_in_training
    ...
    # ═══════════════════════════════════════════════════
    # RUN B: Held-out cancer + non-cancer (real-world ratio)
    # ═══════════════════════════════════════════════════
    # Only keep non-cancer from unseen (in case any have LABEL=1)
    unseen_non_cancer = unseen[unseen['LABEL'] == 0]

    # Align to common columns
    # For training cancer, only include columns that exist
    training_cancer_cols = [c for c in common_cols if c in training_cancer.columns]
    unseen_cols = [c for c in common_cols if c in unseen_non_cancer.columns]

    # Use intersection of available columns
    align_cols = sorted(set(training_cancer_cols) & set(unseen_cols))

    training_cancer_aligned = training_cancer[align_cols].copy()
    unseen_aligned = unseen_non_cancer[align_cols].copy()
