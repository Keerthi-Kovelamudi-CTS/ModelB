"""
Try to reach 75% sensitivity / 65% specificity
by using CatBoost (highest recall) in the ensemble
and testing weighted combinations on existing test_predictions.csv.

Run after 1_run_modeling.py. Reads ensembleresults/{window}/test_predictions.csv.
Usage: python 2_try_ensembles_for_target.py [--window 3mo|6mo|12mo|12mo_250k]  (default: 12mo_250k)
"""
...
RESULTS_DIR = os.path.join(SCRIPT_DIR, 'ensembleresults', RUN_SUBFOLDER)
...
TARGET_SENS, TARGET_SPEC = 0.75, 0.65
best_result = None
...
# Collect ALL (ensemble, threshold) options meeting Sens≥75% & Spec≥65% on test set
...
if all_options:
    ...
    print(f"\n{'='*70}")
    print("ALL THRESHOLDS MEETING Sens≥75% & Spec≥65% (pick by metrics)")
    print(f"{'='*70}")
...
if best_result:
    ...
    print(f"  Sensitivity: {r['sens']:.1%} (target >=75%)")
    print(f"  Specificity: {r['spec']:.1%} (target >=65%)")
    print(f"  PPV: {r['ppv']:.1%}")
