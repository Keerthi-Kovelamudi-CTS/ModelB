...
# Train/val/test split: 25% train, 10% val, 65% test
TEST_SIZE = 0.65   # 65% test
VAL_FRACTION = 10/35  # 10% val of (train+val); 25% train, 10% val, 65% test

# Output under subfolder encoding split (test-train-val: 65-25-10)
...
ENSEMBLERESULTS_DIR = os.path.join(SCRIPT_DIR, 'ensembleresults', RUN_SUBFOLDER)
...
print("=" * 70)
print(f"Window: {WINDOW}  |  Target: ≥75% sensitivity, ≥65% specificity")
print(f"Output: results/{RUN_SUBFOLDER}/, models/{RUN_SUBFOLDER}/, ensembleresults/{RUN_SUBFOLDER}/")
print(f"Input:  {INPUT_FILE}")

if not os.path.isfile(INPUT_FILE):
    raise FileNotFoundError(
        f"Input not found: {INPUT_FILE}\n"
        f"Run the FE pipeline first (e.g. 5_feature_cleanup.py --window {WINDOW}) to create bladder_feature_matrix_{WINDOW}_cleaned.csv"
    )
...
# ══════════════════════════════════════════════════════
print("\n" + "=" * 70)
print(f"2. TRAIN / VAL / TEST SPLIT (25/10/65) [output: {RUN_SUBFOLDER}]")
print("=" * 70)
...
# Finer grid (0.025 steps) — Prec=PPV, Recall=Sensitivity
print(f"\n{'Thresh':>7s} {'PPV':>8s} {'Sens':>8s} {'Spec':>8s} {'NPV':>8s} {'F1':>8s} {'F2':>8s} {'TP':>5s} {'FN':>5s} {'FP':>5s}")
print("-" * 88)
...
for t in np.arange(0.05, 0.75, 0.025):
    ...
    print(f"{t:7.3f} {prec:8.4f} {rec:8.4f} {spec:8.4f} {npv:8.4f} {f1:8.4f} {f2:8.4f} {tp:5d} {fn:5d} {fp:5d}")

print(f"\nBest F1: {best_f1:.4f} @ {best_f1_t:.3f}")
print(f"Best F2: {best_f2:.4f} @ {best_f2_t:.3f}")

# Summary: Sens / Spec / PPV / NPV at selected thresholds (same model, test set)
print(f"\n--- Sens / Spec / PPV / NPV at selected thresholds ({best_model_name}, TEST) ---")
print(f"{'Thresh':>7s} {'Sens':>8s} {'Spec':>8s} {'PPV':>8s} {'NPV':>8s}")
print("-" * 42)
for t in [0.015, 0.02, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50]:
    yp = (y_prob_best >= t).astype(int)
    tp = ((yp==1)&(y_test==1)).sum()
    fn = ((yp==0)&(y_test==1)).sum()
    fp = ((yp==1)&(y_test==0)).sum()
    ...
    npv = tn/(tn+fn) if (tn+fn)>0 else 0
    print(f"{t:7.3f} {sens:7.1%} {spec:7.1%} {ppv:7.1%} {npv:7.1%}")
...
# ══════════════════════════════════════════════════════
# 10b. TARGET OPERATING POINT (Sens≥75%, Spec≥65%)
#     Optional: calibrate probs on val so threshold generalizes better
# ══════════════════════════════════════════════════════
print("\n" + "=" * 70)
print("10b. TARGET OPERATING POINT (Sens≥75%, Spec≥65%)")
print("     Threshold from VALIDATION; test metrics reported")
print("=" * 70)

val_preds_10b = {k: np.array(v, copy=True) for k, v in val_preds.items()}
test_preds_10b = {k: np.array(v, copy=True) for k, v in test_preds.items()}
if USE_CALIBRATION_FOR_TARGET:
    print("  Calibrating predictions on validation set for threshold search...")
    for name in list(val_preds_10b.keys()):
        cal = LogisticRegression(C=1e10, max_iter=1000, random_state=SEED)
        cal.fit(val_preds_10b[name].reshape(-1, 1), y_val)
        val_preds_10b[name] = cal.predict_proba(val_preds_10b[name].reshape(-1, 1))[:, 1]
        test_preds_10b[name] = cal.predict_proba(test_preds_10b[name].reshape(-1, 1))[:, 1]
    print("  Done. Using calibrated probs for 75/65 threshold and test metrics.")
...
target_ops = {}
for name in val_preds_10b:
    valid = []
    for t in threshold_grid:
        pv = (val_preds_10b[name] >= t).astype(int)
        tp = ((pv==1)&(y_val==1)).sum()
        fn = ((pv==0)&(y_val==1)).sum()
        fp = ((pv==1)&(y_val==0)).sum()
        tn = n_val_neg - fp
    ...
    if valid:
        # Prefer sensitivity (75+ ideally 80+) while keeping spec ≥ 65%
...
for name in list(target_ops.keys()):
    ...
    pt = (test_preds_10b[name] >= t).astype(int)
...
# Collect ALL (model, threshold) options meeting Sens≥75% & Spec≥65% on VAL; compute TEST metrics for each
...
if target_ops:
    ...
    print(f"\n★ Recommended (highest sens among those meeting Sens≥75% & Spec≥65%): {best_for_target} @ {rec_info['threshold']:.3f}")
    ...
    pt = (test_preds_10b[best_for_target] >= best_t_fb).astype(int)
    tp = ((pt==1)&(y_test==1)).sum()
    fn = ((pt==0)&(y_test==1)).sum()
    fp = ((pt==1)&(y_test==0)).sum()
    tn = n_neg - fp
...
y_prob_cost = test_preds_10b[best_for_target] if USE_CALIBRATION_FOR_TARGET else test_preds[best_for_target]
...
# One-line summary for 75/65 target (use this in deployment)
print("\n" + "=" * 70)
print("FOR 75/65 TARGET — USE IN DEPLOYMENT")
print("=" * 70)
print(f"  Model:      {best_for_target}")
print(f"  Threshold:  {rec_info['threshold']:.4f}")
print(f"  Test Sens: {rec_info['test_sens']:.1%}  (target ≥75%)")
print(f"  Test Spec: {rec_info['test_spec']:.1%}  (target ≥65%)")
print(f"  Target met: {target_met}")
if USE_CALIBRATION_FOR_TARGET:
    print("  (Threshold is on calibrated probs; calibrate new predictions the same way before applying.)")
print("=" * 70)
