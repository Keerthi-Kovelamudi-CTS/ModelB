# ═══════════════════════════════════════════════════════════════
# PROSTATE CANCER — Prostate_B+C_clinical PIPELINE CONFIGURATION
#
# Same dual-horizon design as Prostate_B+C, but with CLINICALLY-GROUNDED
# temporal binning (acute / subacute / recent_past / historical) instead
# of arbitrary A/B halves.
#
...
#
# Only the FE temporal-binning scheme differs:
#   - B+C used:           A (earlier half) / B (recent half) — arbitrary halves
#   - B+C_clinical uses:  acute / subacute / recent_past / historical
#                          (clinically-grounded, fixed 3/12/24-month boundaries)
# ═══════════════════════════════════════════════════════════════
...
# Clinically-grounded temporal bins (MONTHS_BEFORE_INDEX boundaries).
# FIXED across all 3 model variants — no per-variant scaling needed.
# Variants with shorter lookback simply produce empty/zero values for
# the older bins, which is informative (and dropped by cleanup if constant).
#
# Boundaries (months before anchor):
#   acute       : 0  - 3      (red-flag presentation, recent referral)
#   subacute    : 3  - 12     (subacute trajectory, ongoing workup)
#   recent_past : 12 - 24     (past-year baseline)
#   historical  : 24+         (long-term medical history)
#
# Clinical justification for the boundaries:
#   3 months  = NICE 2-week-wait + typical PSA re-test interval
#   12 months = annual primary-care review cycle
#   24 months = sufficient longitudinal baseline before "historical"
CLINICAL_BIN_BOUNDARIES = {
    "acute":       (0, 3),
    "subacute":    (3, 12),
    "recent_past": (12, 24),
    "historical":  (24, None),   # None = open-ended (24+)
}
# Order matters: from most recent → most historical (for adjacent-bin features)
CLINICAL_BIN_NAMES = ["acute", "subacute", "recent_past", "historical"]

# Adjacent-bin pairs for delta / acceleration features (recent → earlier).
# e.g. acceleration_acute_vs_subacute = (count_acute > count_subacute) ? 1 : 0
ADJACENT_BIN_PAIRS = [
    ("acute",       "subacute"),
