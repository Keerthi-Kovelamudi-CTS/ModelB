# Prostate B + C — Clinical-Bin Variant

Same dual-horizon design as `Prostate_B+C/`, but with **clinically-grounded temporal binning** instead of arbitrary A/B halves. Built to answer the question: does clinically-meaningful binning (acute / subacute / recent_past / historical) improve interpretability and/or accuracy versus arbitrary halves?
...
## Clinical temporal bins (the key difference)

Fixed boundaries (in months before anchor), NOT scaled per variant:

| Bin | Window | Clinical meaning |
|---|---|---|
| **acute** | 0–3 months | Red-flag window; recent symptoms, PSA spike, urgent referral |
| **subacute** | 3–12 months | Subacute trajectory; recent abnormalities, ongoing workup |
| **recent_past** | 12–24 months | Past-year baseline; established conditions |
| **historical** | 24+ months | Long-term medical history; chronic comorbidities |

Variants with shorter lookback (e.g. `1mo_12mo`) simply produce empty `historical` values — informative, naturally handled by cleanup.

Feature names become self-documenting: `OBS_PSA_count_acute`, `OBS_HAEMATURIA_new_in_acute`, `LAB_PSA_delta_acute_minus_subacute`, etc.
