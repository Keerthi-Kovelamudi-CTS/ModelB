...
# Pull the variant-appropriate A/B midpoint from config so the recent-vs-earlier
# split is meaningful for ALL three variants (1mo_12mo only has 12 months of
# data — a hardcoded 30-month midpoint would put 100% of events in window B).
try:
    import config as _cfg
    TIME_WINDOW_MID_MONTHS = _cfg.TIME_WINDOW_MID_PER_WINDOW.get(_WINDOW, 30)
except Exception:
    TIME_WINDOW_MID_MONTHS = 30   # fallback for direct script use
...
def assign_time_window(months, mid_months=None):
    """Bin event-age into recent (B) / earlier (A) buckets.

    Events 1..mid_months → "B" (recent); events > mid_months → "A" (earlier).
    `mid_months` defaults to the module-level TIME_WINDOW_MID_MONTHS (set from
    WINDOW env var at import time), but callers can pass an explicit value to
    avoid env-var coupling — required when one process processes multiple
    windows in sequence (e.g. 4_transform_features_json.py).
    """
    if mid_months is None:
        mid_months = TIME_WINDOW_MID_MONTHS
...
def main():
    ...
    ap.add_argument("--prefix",  default=f"prostate_{_WINDOW}",     help="output filename prefix (defaults to prostate_{\})")
    args = ap.parse_args()
