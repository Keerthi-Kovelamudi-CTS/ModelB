# Prostate B + C — No-Bins Variant

Same dual-horizon design as `Prostate_B+C/`, but with **no temporal binning at all**. Every event collapses into one bin called `'all'`. Temporal information is captured entirely by continuous features (slopes, time-decay, time_span_years, frequency_per_year, days_since_last, monthly bins, rolling 3-month windows).

Built to answer the question: do bin-count features (count_acute, count_subacute, etc.) actually add signal over continuous temporal features alone, or are they redundant?

- **Model B (12mo)** — Early prediction. Trained on data ending 12 months before diagnosis.
- **Model C (1mo)** — Acute alert. Trained on data ending 1 month before diagnosis.

The two models run in parallel per patient, OR-combined: if **either** fires → overall result = `Cancer`.

## No-bins design (the key difference)

`config.py` sets:
```python
CLINICAL_BIN_BOUNDARIES = {"all": (0, None)}   # single open-ended bin
CLINICAL_BIN_NAMES      = ["all"]
ADJACENT_BIN_PAIRS      = []                   # → acceleration features no-op
```

Result:
- Per-bin loops still run, but emit ONE feature per category — e.g. `OBS_PSA_count_all` rather than the 4 features `count_{acute,subacute,recent_past,historical}`.
- Acceleration / delta / new_in_acute features are skipped entirely (empty pair list).
- Continuous temporal features still computed: `LAB_PSA_slope`, `LAB_PSA_last`, `OBS_PSA_time_span_years`, `OBS_PSA_frequency_per_year`, `OBS_PSA_days_since_last`, `DECAY_LUTS`, monthly bin counts, rolling 3M windows, trajectory slopes — these carry all the temporal signal.

Feature count drops by ~30-40% versus the binned variants. If AUC stays within noise, the simpler design wins.

## Variant comparison

| Folder | Binning | Bin-derived features | Use case |
|---|---|---|---|
| `Prostate_B+C/` | A / B halves (arbitrary midpoint) | count_A, count_B, accel, new_in_B | Original lookback experiment |
| `Prostate_B+C_clinical/` | 4 clinical bins (acute/subacute/recent_past/historical) | count_acute, …, accel_acute_vs_subacute, new_in_acute | Clinically-grounded windowing |
| **`Prostate_B+C_simple/` (this)** | None — single 'all' bin | count_all only (effectively == count_total) | Parsimony test: do bins add anything? |
