...
def build_placeholder_rows(unified_patients, sex_lookup, age_lookup,
    ...
    For the no-bins variant, every event (real or placeholder) lands in
    the single bin called 'all'. The 6-month offset is still sensible
    because it puts the placeholder safely inside even the shortest
    lookback (1mo_12mo, which goes back to month 12).
    """
    rows = []
    PLACEHOLDER_OFFSET_MONTHS = 6   # → safely inside even the 12-month lookback
    ...
    for pid, anchor_date, label in unified_patients.itertuples(index=False):
        ...
        rows.append({
            ...
            "TIME_WINDOW": "all",   # single-bin (no-bins variant)
