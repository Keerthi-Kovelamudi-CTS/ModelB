# ═══════════════════════════════════════════════════════════════
# PROSTATE CANCER — Prostate_B+C_simple PIPELINE CONFIGURATION
#
# Same dual-horizon design as Prostate_B+C, but with NO TEMPORAL BINNING.
# Every event collapses into one bin called 'all'. Temporal information
# is captured ENTIRELY by continuous features (slopes, time_span_years,
# frequency_per_year, days_since_last, time-decay, trajectories).
#
# Three models, one unified cohort, one shared split:
...
#   - B+C            : 2 bins (A / B halves) — arbitrary midpoint
#   - B+C_clinical   : 4 bins (acute / subacute / recent_past / historical)
#   - B+C_simple     : 1 bin  ('all') — i.e. NO bins; totals + continuous only
#
# Rationale: at our AUC ceiling, slopes + decay + last-value features may
# already capture everything bin-counts add. This variant tests that — if
# AUC ≈ binned variants, the simpler design wins on parsimony and clarity.
# ═══════════════════════════════════════════════════════════════
...
# ─── No-bins configuration ────────────────────────────────────
# Single bin called 'all' that covers the entire lookback (0 → None).
# All events fall into this one bin → per-bin loops still work, but
# produce ONE feature per category (e.g., OBS_PSA_count_all).
#
# Temporal patterns are captured by continuous features instead of bins:
#   - count_total, has_ever, time_span_years, frequency_per_year
#   - last/first/min/max/mean/slope per lab category
#   - days_since_last, days_since_first
#   - exponential time-decay weighted counts
#   - trajectory slopes, monthly bins, rolling 3-month windows
#
# ADJACENT_BIN_PAIRS is empty → acceleration/delta features are no-ops.
# This is intentional: the comparison test asks whether bin-count features
# add value over continuous temporal features alone.
CLINICAL_BIN_BOUNDARIES = {
    "all": (0, None),
}
CLINICAL_BIN_NAMES = ["all"]
ADJACENT_BIN_PAIRS = []   # no acceleration/delta features in this variant
