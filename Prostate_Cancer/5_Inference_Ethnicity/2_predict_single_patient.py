...
def predict(fm, artifacts):
    ...
    # Force zero-risk on zero-feature patients (no clinical evidence → no risk).
    # Training never saw all-zero vectors; boosters extrapolate them unstably.
    clinical_cols = [c for c in selected if c not in ('AGE_AT_INDEX', 'AGE_BAND')]
    if clinical_cols and float(X[clinical_cols].abs().sum(axis=1).iloc[0]) == 0.0:
        logger.info("  Zero-feature patient → forced to risk=0 (no clinical evidence)")
        risk_score = 0.0
