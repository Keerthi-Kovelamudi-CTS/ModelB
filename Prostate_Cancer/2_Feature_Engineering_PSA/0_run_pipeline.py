...
def run_feature_engineering(cfg):
    ...
    for window in cfg.WINDOWS:
        ...
        # Skip if final matrix already exists (resume after partial run)
        out_path = cfg.FE_RESULTS / window / f'feature_matrix_final_{window}.csv'
        if out_path.exists():
            size_mb = out_path.stat().st_size / (1024 * 1024)
            logger.info(f"\n[{window}] SKIP — {out_path.name} already exists ({size_mb:.0f} MB)")
            saved[window] = ('cached', size_mb)
            continue

        # ── Load this window's _dropped data ──────────────────
        obs_path = cfg.BASE_PATH / window / f'{cfg.DATA_PREFIX}_{window}_obs_dropped.csv'
        med_path = cfg.BASE_PATH / window / f'{cfg.DATA_PREFIX}_{window}_med_dropped.csv'
        try:
        ...
        del adv
        if cfg.SAVE_INTERMEDIATES:
            fm.to_csv(cfg.FE_RESULTS / window / f'feature_matrix_enhanced_{window}.csv')
        ...
        del mega
        if cfg.SAVE_INTERMEDIATES:
            fm.to_csv(cfg.FE_RESULTS / window / f'feature_matrix_mega_{window}.csv')
