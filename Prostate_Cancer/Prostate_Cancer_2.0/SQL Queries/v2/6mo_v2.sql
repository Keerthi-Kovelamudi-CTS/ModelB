/*
 * Prostate cancer prediction — 6mo window — Approach B (v2)
 *
 ...
 *                 to match cancer year distribution at 1:10 ratio. NO curated-codes
 ...
 *
 * LOOKBACK WINDOW (6mo prediction = 6-month gap before anchor):
 *   - Events in [anchor - 60 months, anchor - 6 months]
 ...
 *   PATIENT_GUID, CANCER_CLASS, CANCER_ID, SEX, PATIENT_AGE, PATIENT_ETHNICITY,
...
WITH params AS (
  SELECT
    ...
    6                 AS months_before,               -- 6mo gap before anchor (matches 6mo-prediction)
    10                AS min_obs_events_per_patient,  -- symmetric (cancer + non-cancer): ≥10 events of any kind in 60mo (general data, not curated)
    1                 AS min_med_events_per_patient,  -- row-level filter on med output (not cohort gate)
    10                AS non_cancer_ratio,            -- 1:10 cancer:non-cancer training distribution
...
/* ═══════════════════════════════════════════════════════════════════════════
   Step 5: 1:10 year-stratified non-cancer sample
   Within each anchor year, randomly pick (10 × cancer_count[year]) non-cancer.
