# ═══════════════════════════════════════════════════════════════
# BREAST CANCER — PIPELINE ORCHESTRATOR
...
def run_feature_engineering(cfg):
    ...
    """
    import gc
    _pipeline = import_module('1_pipeline_blocks')
    _cancer = import_module('2_prostate_features')
    ...
    for window in cfg.WINDOWS:
        ...
        clin_features = build_clinical_features(clin, cfg, window.upper())
...
def run_cleanup(cfg):
    """Run feature cleanup (step 3 in new layout)."""
    import_module('3_cleanup').run_cleanup(cfg)



def run_write_to_bq(cfg):
    """Run BQ feature-store write (step 4 in new layout) — mirrors lung's write_to_bigQ pattern."""
    import_module('4_transform_features_json').run_write_to_bq(cfg)


def main():
    parser = argparse.ArgumentParser(description=f'{config.CANCER_NAME.title()} Cancer FE Pipeline')
    parser.add_argument('--step', choices=['sanity', 'clean', 'fe', 'cleanup', 'bq', 'all'],
    ...
    if args.step in ('sanity', 'all'):
        import_module('_sanity_check').run_sanity_check(cfg)

    if args.step in ('clean', 'all'):
        import_module('_clean_data').run_clean_data(cfg)
