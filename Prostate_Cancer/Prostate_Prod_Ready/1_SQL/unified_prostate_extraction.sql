/*
 * ═══════════════════════════════════════════════════════════════════════════
 *  PROSTATE-PROD-READY SQL — adapted from lung/sql/unified_cancer_noncancer34_v2.sql
 * ═══════════════════════════════════════════════════════════════════════════
 *
 *  Changes from lung base:
 *    1. target_cancer_pattern: '%lung%' -> '%prostate%'
 *    2. Sex filter: pp.sex='M' applied to BOTH cohorts (prostate is male-only)
 *
 *  Kept from lung (deliberately, per design):
 *    - NO anchor metadata in output (anchor derived in FE preprocess fallback)
 *    - NO year-stratified sampling (lung uses random; flag for review)
 *    - Two-tier ethnicity (LABEL_16, LABEL_6) via LEFT JOIN Patient_Ethnicity
 *    - 14-SNOMED admin exclusion list
 *    - Same min_obs_events=50, min_med_events=10 thresholds
 *
...
target_cancer_patients AS (
  ...
  FROM (
    ...
    JOIN patients pp ON pp.patient_guid = mh.patient_guid   -- [PROSTATE] sex filter
    CROSS JOIN params param
    WHERE mh.effective_date IS NOT NULL
      AND pp.sex = 'M'                                       -- [PROSTATE] male-only
...
non_cancer_patients AS (
  ...
  FROM medical_history mh
  JOIN patients pp ON pp.patient_guid = mh.patient_guid    -- [PROSTATE] sex filter
  WHERE pp.sex = 'M'                                        -- [PROSTATE] male-only
    AND NOT EXISTS (
