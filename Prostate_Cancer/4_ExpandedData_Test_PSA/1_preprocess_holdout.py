...
# MONTHS_BEFORE_INDEX values in the BQ-exported training data, so these
# are the actual bounds the model trained on.
# Pattern: B (recent) = [window_months, midpoint),
#          A (earlier) = [midpoint, 49).
# Total lookback: events 1–48 months before INDEX_DATE.
# Midpoint shifts with window size (≈ ceil((W + 48) / 2)).
WINDOW_DEFS = {
    "1mo":  {"B": (1,  25), "A": (25, 49)},
    "2mo":  {"B": (2,  25), "A": (25, 49)},
    "3mo":  {"B": (3,  26), "A": (26, 49)},
    "6mo":  {"B": (6,  27), "A": (27, 49)},
    "9mo":  {"B": (9,  29), "A": (29, 49)},
    "12mo": {"B": (12, 30), "A": (30, 49)},
}
INDEX_DATE = pd.Timestamp("2026-04-01")  # matches BQ extract cutoff (max INDEX_DATE in training data)
CHUNKSIZE = 500_000
