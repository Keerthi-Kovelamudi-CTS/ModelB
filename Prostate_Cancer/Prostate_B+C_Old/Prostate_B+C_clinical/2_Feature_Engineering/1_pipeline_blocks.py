...
def build_clinical_features(clin_df, cfg, window_name='12mo'):
    ...
    # "New in acute" — category that appeared in the acute bin but NOT in any earlier bin.
    # Clinically: "first-time presentation of this symptom in the last 3 months."
    obs_new_acute = pd.DataFrame(index=obs_total.index)
    df_acute = obs_per_bin.get('acute')
    if df_acute is not None:
        for cat in obs_categories:
            col_acute = f"OBS_{cat}_count_acute"
            ...
            for earlier_b in ['subacute', 'recent_past', 'historical']:
            ...
            obs_new_acute[f"OBS_{cat}_new_in_acute"] = (
    ...
    # "New symptom categories in acute" — categories appearing in acute bin but in NO earlier bin
    symptom_cats_acute = symptom_df[symptom_df['TIME_WINDOW'] == 'acute'].groupby('PATIENT_GUID')['CATEGORY'].apply(set)
    earlier_cats = (
        symptom_df[symptom_df['TIME_WINDOW'].isin(['subacute', 'recent_past', 'historical'])]
    ...
    symptom_new_acute_count.name = 'AGG_new_symptom_cats_in_acute'
...
def build_medication_features(med_df, cfg):
    ...
    # New-in-acute: med category appeared in acute bin but in no earlier bin
    med_new_acute = pd.DataFrame(index=med_total.index)
    df_acute = med_per_bin.get('acute')
    if df_acute is not None:
        for cat in med_categories:
            col_acute = f"MED_{cat}_count_acute"
            if col_acute not in df_acute.columns:
                continue
            earlier_sum = pd.Series(0, index=df_acute.index)
            for earlier_b in ['subacute', 'recent_past', 'historical']:
            ...
            med_new_acute[f"MED_{cat}_new_in_acute"] = (
                (df_acute[col_acute] > 0) & (earlier_sum == 0)
            ).astype(int)
...
def build_advanced_features(clin_df, med_df, existing_fm, window_name, cfg):
    ...
    for group_name, cats in cfg.CLUSTER_DEFINITIONS:
        ...
        group_B = group_df[group_df['TIME_WINDOW'] == 'acute'].groupby('PATIENT_GUID').size()
    ...
    visits_A = clin_windowed[clin_windowed['TIME_WINDOW'] == 'subacute'].groupby('PATIENT_GUID')['EVENT_DATE'].nunique()
    visits_B = clin_windowed[clin_windowed['TIME_WINDOW'] == 'acute'].groupby('PATIENT_GUID')['EVENT_DATE'].nunique()
    ...
    pain_A = pain_med_df[pain_med_df['TIME_WINDOW'] == 'subacute'].groupby('PATIENT_GUID').size()
    pain_B = pain_med_df[pain_med_df['TIME_WINDOW'] == 'acute'].groupby('PATIENT_GUID').size()
    ...
    med_cats_A = med_windowed[med_windowed['TIME_WINDOW'] == 'subacute'].groupby('PATIENT_GUID')['CATEGORY'].nunique()
    med_cats_B = med_windowed[med_windowed['TIME_WINDOW'] == 'acute'].groupby('PATIENT_GUID')['CATEGORY'].nunique()
...
def build_acceleration_features(clin_df, med_df, existing_fm, window_name, cfg):
    ...
    obs_AB = clin[clin['TIME_WINDOW'].isin(_bin_names(cfg))].copy()
    obs_A = obs_AB[obs_AB['TIME_WINDOW'] == 'subacute']
    obs_B = obs_AB[obs_AB['TIME_WINDOW'] == 'acute']
    ...
    def _velocity_recent_vs_lifetime(group):
        ...
        g_recent = g[g['TIME_WINDOW'] == 'acute']
