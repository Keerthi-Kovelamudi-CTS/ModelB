"""
SHAP explainability for v3+ models (multi-seed + per-band calibration).
...
sys.path.insert(0, str(PROJECT_ROOT / "2_Feature_Engineering"))
from _calibrator import CalibratorWrapper  # noqa: F401
import config as fe_config  # for WINDOWS list
...
def shap_for_model(model, X):
    """SHAP array for XGBoost — uses native predict(pred_contribs=True) to bypass
    shap.TreeExplainer's XGBoost-≥2.x loader bug ("could not convert '[5E-1]' to float").

    Returns 2D shape (n_samples, n_features), positive class only. Last column from
    XGBoost's contribs is the bias term — we drop it.
    """
    import xgboost as xgb
    try:
        booster = model.get_booster()
        dmat = xgb.DMatrix(X, feature_names=list(X.columns), enable_categorical=True)
    except Exception:
        # Non-XGBoost model — fall back to shap.TreeExplainer
...
def main():
    ...
    p.add_argument("--window", required=True, choices=fe_config.WINDOWS)
