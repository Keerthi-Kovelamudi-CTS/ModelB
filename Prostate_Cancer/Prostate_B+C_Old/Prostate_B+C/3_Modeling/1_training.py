...
#
# Cohort source: 1:1 balanced via SQL Queries/v4/*_v4.sql (non_cancer_ratio = 1).
...
BALANCE_TRAIN = getattr(fe_config, 'BALANCE_TRAIN', True)
...
import json  # used to read the shared_split file
...
def train_val_test_split(X, y, seed):
    """[LEGACY] Stratified random split. Kept for back-compat.

    In Prostate_B+C the SHARED split (shared_split.json) is the canonical
    source of truth — see split_using_shared_assignment() below. Use that
    when training across multiple windows to keep Patient X in the same
    split for all 3 models.
    """
...
def split_using_shared_assignment(X, y, shared_split, balance_train=False, balance_seed=42):
    """Apply the unified shared_split.json assignment to one window's feature matrix.

    X must have PATIENT_GUID as the index (or be reset to it). Filters X+y down
    to the patients in this window AND in each split, ordered to match the
    shared_split.json file. Patient X ends up in the same split (train/val/test)
    for every window using this function.

    If balance_train=True, ALL THREE splits (train + val + test) are downsampled to 1:1
    cancer:non-cancer. Recalibrate to production prevalence after picking a winner.

    Returns (X_train, X_val, X_test, y_train, y_val, y_test) — same shape as
    ...
    if X.index.name != "PATIENT_GUID" and "PATIENT_GUID" not in X.columns:
        raise ValueError(
            "split_using_shared_assignment requires PATIENT_GUID as index or column"
        )
    if "PATIENT_GUID" in X.columns:
        X = X.set_index("PATIENT_GUID")

    # `y` may be a 1-d array aligned with X — convert to a Series indexed by PATIENT_GUID
    ...
    def _balance_split(X_, y_, split_name, seed):
        n_pos = int((y_ == 1).sum())
        n_neg = int((y_ == 0).sum())
        ...
        logger.info(
            f"  {split_name} balanced 1:1: {target}+{target}={2*target} patients (was {n_pos}+{n_neg})"
        )
        return X_.loc[keep], y_.loc[keep]

    if balance_train:
        # balance_train=True now balances ALL splits (train + val + test) to 1:1.
        # Same patient-level deterministic seed across windows so the same patient
        # ends up in the same split for every model variant (shared_split contract preserved).
...
def run_modeling():
    ...
    for window in WINDOWS:
        ...
        # ─── SHARED split — Patient X is in same split for all 3 models ──
        # Read shared_split.json (built ONCE on the unified cohort by _shared_split.py).
        # Falls back to per-window stratified split if the shared file isn't present yet
        # (e.g. during initial bootstrapping before shared_split.json exists).
        SPLIT_SEED = 42
        if SHARED_SPLIT_PATH.exists():
            shared_split = json.load(open(SHARED_SPLIT_PATH))
            X_train, X_val, X_test, y_train, y_val, y_test = split_using_shared_assignment(X, y, shared_split, balance_train=BALANCE_TRAIN)
            logger.info(
