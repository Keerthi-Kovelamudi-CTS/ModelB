/*
 * Prostate cancer prediction — 1mo window — Approach B (v4)
 *
 ...
 *   - Events in [anchor - 60 months, anchor - 1 month]
...
WITH params AS (
  SELECT
    ...
    1                 AS years_before,                -- 12mo lookback (NEW — short variant)
    ...
    5                 AS non_cancer_ratio,            -- 5x non-cancer oversample (post-intersection ≈ 1:2 cancer:non-cancer; train downsampled to 1:1)
...
patients AS (
  -- Patient_Ethnicity lives in `prj-cts-ai-dev-sp.EMIS_BULK_DATA_temp`
  -- (NOT in `prj-cts-ai-dev-sp.EMIS_BULK_DATA_temp` — that's
  --  where the earlier "removed" comment came from).
  SELECT
    pa.patient_guid,
    pa.sex,
    pa.date_of_birth,
