...
def _score(feature_vector, cfg, seed_artifacts):
    ...
    Returns (risk_proba, decision, age_band, insufficient_data).
    ...
    if insuf:
        risk_proba = 0.0
        decision = 0
    return risk_proba, decision, age_band, insuf
...
def main():
    ...
    rows = []
    for i, pg in enumerate(test_guids):
        (a_p, a_d, a_band, a_insuf) = cells_A[i]
        (b_p, b_d, b_band, b_insuf) = cells_B[i]
        (c_p, c_d, c_band, c_insuf) = cells_C[i]
        (d_p, d_d, d_band, d_insuf) = cells_D[i]
