...
def build_cancer_specific_features(clin_df, med_df, existing_fm, window_name, cfg=None):
    ...
    # ══════════════════════════════════════════════════════════
    # BLOCK 17: ANAEMIA TRAJECTORY + SYMPTOM-ONLY PATHWAY
    # Targets specific failure modes found in the 2×2 dual-horizon evaluation:
    #  - 30 false positives where the 12mo model fired on elderly + Hb-low
    #    that turned out to be CHRONIC (non-cancer) anaemia.
    #  - 10 missed cancers (FN) that had NO PSA history, so every PSA-conditioned
    #    feature was zero. Symptom-only cluster catches the non-PSA pathway.
    # ══════════════════════════════════════════════════════════
    logger.info(f"  BLOCK 17: Anaemia trajectory + symptom-only pathway...")

    # ── 17a: Hb trajectory — chronic vs acute decline ──
    # Hb in EMIS is g/L (typically 100-180). Cutoffs: <120 = mild anaemia (male),
    # drop ≥20 g/L = 2 g/dL = clinically meaningful decline.
    hb_traj_lab = clin[(clin['CATEGORY'] == 'FBC_HAEMATOLOGY') &
                        pd.to_numeric(clin['VALUE'], errors='coerce').notna() &
                        clin['TERM'].astype(str).str.contains('Haemoglobin', case=False, na=False)].copy()
    if len(hb_traj_lab) > 0:
        hb_traj_lab['VALUE'] = pd.to_numeric(hb_traj_lab['VALUE'], errors='coerce')
        hb_traj_lab['EVENT_DATE'] = pd.to_datetime(hb_traj_lab['EVENT_DATE'], errors='coerce')
        hb_traj_lab = hb_traj_lab.dropna(subset=['EVENT_DATE'])

        def _hb_traj_features(group):
            """Per patient: (decline_rate_per_year, chronic_low_24mo, acute_decline_12mo).
            decline_rate positive = Hb falling; chronic = ≥24mo span below 120 g/L;
            acute = ≥20 g/L drop within last 12 months."""
            if len(group) < 2:
                return pd.Series({'decline_rate_year': 0.0, 'chronic_low_24mo': 0, 'acute_decline_12mo': 0})
            g = group.sort_values('EVENT_DATE')
            x = (g['EVENT_DATE'] - g['EVENT_DATE'].min()).dt.days.astype(float).values
            y = g['VALUE'].astype(float).values
            if len(np.unique(x)) > 1:
                slope_per_day = np.polyfit(x, y, 1)[0]
                decline_per_year = -float(slope_per_day) * 365.25
            else:
                decline_per_year = 0.0
            below_120 = g[g['VALUE'] < 120].sort_values('EVENT_DATE')
            chronic = 0
            if len(below_120) >= 2:
                span_days = (below_120['EVENT_DATE'].iloc[-1] - below_120['EVENT_DATE'].iloc[0]).days
                chronic = 1 if span_days >= 730 else 0
            last_date = g['EVENT_DATE'].max()
            recent = g[g['EVENT_DATE'] >= last_date - pd.Timedelta(days=365)].sort_values('EVENT_DATE')
            acute = 0
            if len(recent) >= 2:
                drop = recent['VALUE'].iloc[0] - recent['VALUE'].iloc[-1]
                acute = 1 if drop >= 20 else 0
            return pd.Series({
                'decline_rate_year': decline_per_year,
                'chronic_low_24mo': chronic,
                'acute_decline_12mo': acute,
            })

        hb_traj = hb_traj_lab.groupby('PATIENT_GUID').apply(_hb_traj_features)
        pf[f'{PREFIX}HB_decline_rate_per_year'] = hb_traj['decline_rate_year'].reindex(pf.index, fill_value=0).astype(float).clip(-50, 50)
        pf[f'{PREFIX}HB_low_chronic_24mo']      = hb_traj['chronic_low_24mo'].reindex(pf.index, fill_value=0).astype(int)
        pf[f'{PREFIX}HB_acute_decline_12mo']    = hb_traj['acute_decline_12mo'].reindex(pf.index, fill_value=0).astype(int)

        # ── 17b: Conditional anaemia firing — only when paired with prostate signal ──
        # Refined version of PROST_AGEX_HB_low: only fires for elderly patients with
        # acute Hb decline AND prostate-specific evidence (raised PSA or high ALP).
        # Targets the 30 12mo FPs that fire on chronic-anaemic-elderly non-cancers.
        psa_high = (psa_max.reindex(pf.index, fill_value=0) > 4.0).astype(int)
        alp_high = pf.get(f'{PREFIX}LAB_alp_high', pd.Series(0, index=pf.index)).astype(int)
        prostate_signal = ((psa_high == 1) | (alp_high == 1)).astype(int)
        pf[f'{PREFIX}AGEX_HB_decline_with_prostate_signal'] = (
            pf[f'{PREFIX}HB_acute_decline_12mo'].astype(int) * prostate_signal
        )
        # Inverse flag: anaemia BUT NO prostate signal (the FP-prone group)
        pf[f'{PREFIX}AGEX_HB_chronic_alone_no_prostate_signal'] = (
            pf[f'{PREFIX}HB_low_chronic_24mo'].astype(int) * (1 - prostate_signal)
        )

    # ── 17c: Symptom-only cancer cluster (no PSA conditioning) ──
    # Targets the 10 missed cancers that had no PSA history. These patients were
    # diagnosed via symptom-driven workup — haematuria, urinary retention, bone pain
    # — without going through the PSA-screening pathway. Flag patients with ≥2 of
    # these symptom categories regardless of PSA status.
    no_psa_symptom_cats = ['HAEMATURIA', 'URINARY_RETENTION', 'PAIN_PELVIC_BONE', 'BONE_MUSCLE']
    sym_no_psa = clin[clin['CATEGORY'].isin(no_psa_symptom_cats)].copy()
    if len(sym_no_psa) > 0:
        sym_count = sym_no_psa.groupby('PATIENT_GUID')['CATEGORY'].nunique()
        pf[f'{PREFIX}SYMPTOM_no_psa_cluster_count']   = sym_count.reindex(pf.index, fill_value=0).astype(int)
        pf[f'{PREFIX}SYMPTOM_no_psa_cluster_2plus']   = (pf[f'{PREFIX}SYMPTOM_no_psa_cluster_count'] >= 2).astype(int)
        pf[f'{PREFIX}SYMPTOM_no_psa_cluster_3plus']   = (pf[f'{PREFIX}SYMPTOM_no_psa_cluster_count'] >= 3).astype(int)
    ...
    for prefix_check, name in [
        ...
        (f'{PREFIX}HB_decline_rate', 'Hb decline rate (chronic vs acute)'),
        (f'{PREFIX}HB_low_chronic', 'Hb chronic low (≥24mo)'),
        (f'{PREFIX}HB_acute_decline', 'Hb acute decline (≥2 g/dL in 12mo)'),
        (f'{PREFIX}AGEX_HB_decline_with_prostate_signal', 'Anaemia + prostate signal (refined FP-fix)'),
        (f'{PREFIX}AGEX_HB_chronic_alone_no_prostate_signal', 'Anaemia alone (no prostate signal — FP-prone)'),
        (f'{PREFIX}SYMPTOM_no_psa_cluster', 'Symptom-only pathway (no-PSA cancers)'),
    ]:
