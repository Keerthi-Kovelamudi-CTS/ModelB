/*╔══════════════════════════════════════════════════════════════════╗
  ...
  ║  Window:  [-25 months, -12 months] before pseudo index date    ║
  ║                                                                 ║
  ║  Negative cohort:                                               ║
  ║    - Male only (prostate is male-only)                          ║
  ║    - Adults (>18)                                               ║
  ║    - No cancer diagnosis of ANY type ever                       ║
  ║    - No palliative care code (1403151000000103)                 ║
  ║    - Age-band + sex matched to prostate cases (10:1)            ║
...
WITH params AS (
  SELECT
    ...
    25                 AS months_lookback_start,
    12                 AS months_lookback_end,
...
cancer_patients AS (
  ...
  INNER JOIN `prj-cts-ai-dev-sp.EMIS_MED_CODES.Dim_Cancer_Codes` AS dcc
    ON mh.snomed_c_t_concept_id = SAFE_CAST(dcc.SNOMED_CODE AS INT64)
  WHERE LOWER(dcc.cancer_id) NOT LIKE '%disease%'
    AND LOWER(dcc.cancer_id) LIKE '%prostate%'
  GROUP BY 1
