"""
╔══════════════════════════════════════════════════════════════════╗
║  PROSTATE — Phase 1.4b — Curation Review                       ║
║                                                                ║
║  Produces _curation_reviews/prostate_curation_review.csv in    ║
...
CONFIG = {
    ...
    "review_tsv":  os.path.join(_SCRIPT_DIR, "Output", "Compare_to_curated",
                                "prostate_combined_review.tsv"),
    "out_dir":     os.path.join(_REPO_ROOT, "_curation_reviews"),
    "out_main":    "prostate_curation_review.csv",
    "out_skipped": "prostate_skipped_no_category.csv",
    "cancer_name": "prostate",     # plain-language disease name
}
...
# but are unrelated to prostate. Force REMOVE_NOISE before dx-pathway runs.
NOISE_OVERRIDES = [
...
OVERRIDES = [
    # Bowel-cancer screening (NOT prostate cancer) — screening invites + negatives
    ...
    # Tumour markers (informative pre-dx workup signals — analogous to PSA)
    ("ca 19-9",                                        "CANCER_MARKERS"),
    ("ca 125",                                         "CANCER_MARKERS"),
    ("ca 15-3",                                        "CANCER_MARKERS"),
    # Prior non-prostate skin cancers — risk factor, not prostate leakage
    ("squamous cell carcinoma",                        "PRIOR_CANCER_HISTORY"),
    ("basal cell carcinoma",                           "PRIOR_CANCER_HISTORY"),
    # Prostate biopsy / cystoscopy / urology referral — strong pre-12mo risk signal
    ("transrectal needle biopsy of prostate",          "UROLOGY_PATHWAY"),
    ("transurethral biopsy prostate",                  "UROLOGY_PATHWAY"),
    ("transperineal needle biopsy of prostate",        "UROLOGY_PATHWAY"),
...
# Cancer-treatment drug leakage — prostate-cancer specific hormone therapy.
...
CANCER_TREATMENT_DRUGS = [
    ...
    # Procedure-coded prostate-ca hormone therapy (gonadorelin/GnRH analogues)
    ...
    ("gonadorelin",                    "gonadorelin (gnrh agonist) — prostate-ca treatment"),
...
DX_PATHWAY_PHRASES = [
    ...
    # (seen in dermatology clinic → noise, not prostate dx-pathway — handled below)
]

# Generic / admin / surveillance noise
NOISE_PHRASES = [
    # ── Topical / dermatology (no systemic relevance for prostate cancer) ──
    ...
    # ── Surgical procedures unrelated to prostate ──
    ...
    # ── Bowel polyps (colorectal; not prostate signal) ──
    ...
    # ── Other meds (treated as noise — not prostate-relevant) ──
    ...
    # ── Surgery / procedures (minor, non-prostate) ──
    ...
    # ── Antihistamines (no prostate signal) ──
    ...
    ("plain x-ray pelvis",     "imaging generic"),
    ("ct scan brain",          "imaging (non-prostate)"),
    ...
    # Non-prostate specialist procedures / referrals
    ...
    ("diagnostic nasendoscopy", "minor procedure (ent)"),
    ("excision biopsy of skin", "minor procedure (skin)"),
    # Generic antibiotics — not prostate-specific (UTI antibiotics handled
    ...
    # Non-prostate imaging
    ("ultrasound of kidney",   "imaging (non-prostate)"),
    # Surgery / procedures
...
# Two tiers:
#   Tier 1 — existing prostate v2 categories (PSA, DRE, LUTS, etc.)
#   Tier 2 — new broader categories observed in pre-12mo data
#            (METABOLIC, CV_RISK, STATIN, etc.)
CATEGORY_KEYWORDS = [
    # ── Tier 1: existing v2 prostate categories ───────────────────
    ("PSA_RATIO",            [r"psa ratio", r"free.*total psa"]),
    ("PSA_FREE",             [r"free psa", r"free.*prostate specific antigen"]),
    ("PSA",                  [r"prostate specific antigen", r"\bpsa\b",
                              r"raised prostate"]),
    ("IPSS",                 [r"\bipss\b", r"international prostate symptom"]),
    ("DRE",                  [r"rectal exam", r"o/e.*pr", r"\bpr\b.*prostat",
                              r"prostate enlarged", r"prostate mass",
                              r"prostatic swelling"]),
    ("PROSTATIC_CONDITIONS", [r"benign prostatic", r"\bbph\b", r"prostatitis",
                              r"hyperplasia of prostate", r"\bprostat"]),
    ("ERECTILE_DYSFUNCTION", [r"erectile", r"impoten", r"psychosexual"]),
    ("SEXUAL_REPRODUCTIVE",  [r"sexual", r"reproductive", r"libido"]),
    ("HAEMATURIA",           [r"haematuria", r"blood in urine",
                              r"red blood cells.*urine"]),
    ("URINARY_RETENTION",    [r"urinary retention", r"retention of urine"]),
    ("LUTS",                 [r"\bluts\b", r"urinary frequency", r"nocturia",
                              r"urinary hesitancy", r"poor stream",
                              r"urinary urgency", r"\bdysuria\b",
                              r"incomplete emptying", r"voiding difficulty",
                              r"overactive bladder", r"urinary incontinence"]),
    ("UTI",                  [r"urinary tract infection", r"\buti\b",
                              r"pyelonephritis", r"cystitis"]),
    ("CATHETER",             [r"catheter"]),
    ...
    ("HORMONAL",             [r"testosterone", r"\blh\b", r"\bfsh\b"]),
    ...
    # ── Med-side categories (prostate-specific) ───────────────────
    ...
    ("THYROID_LAB",          [r"free thyroxine", r"plasma free t4",
                              ...
                              r"\bft4\b", r"\bft3\b"]),
    # ALCOHOL — known minor prostate-cancer risk factor; KEEP
    ...
    # risk factor not prostate leakage)
    ...
    ("UROLOGY_PATHWAY",      [r"needle biopsy of prostate",
                              r"transurethral biopsy prostate",
    ...
    # MUSCULOSKELETAL — back / lumbar pain (could be late prostate signal)
    ("MUSCULOSKELETAL",      [r"pain in lumbar spine",
                              r"supraspinatus tendonitis",
                              r"lumbar spine"]),
    # LUTS — lower urinary tract structure code
    ("LUTS",                 [r"lower urinary tract structure"]),
...
def setup_logging():
    ...
    log_path = os.path.join(CONFIG["out_dir"], f"prostate_curation_log_{ts}.log")
    logging.basicConfig(
        level=logging.INFO,
...
def classify(term: str):
    ...
    # NOISE_OVERRIDES — terms that look like dx-pathway but are non-prostate;
    ...
    # Prostate-cancer treatment drug leakage (GnRH agonists/antagonists,
...
def main():
    logger.info("=" * 80)
    logger.info("PROSTATE — Phase 1.4b — Curation Review")
