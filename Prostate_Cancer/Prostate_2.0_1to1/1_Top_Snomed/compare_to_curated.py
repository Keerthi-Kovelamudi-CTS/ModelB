"""
╔══════════════════════════════════════════════════════════════════╗
║  PROSTATE — Phase 1.4 (Clinical Review Bridge)                ║
║  Compare Ranking.py output vs. existing curated codelist.     ║
║                                                                ║
║  Inputs:                                                       ║
║    - Output/Scores_prostate/prostate_obs_all.csv               ║
║    - Output/Scores_prostate/prostate_meds_all.csv              ║
║    - ../codelists/prostate_curated_codes_v2.tsv                ║
║                                                                ║
║  Outputs (Output/Compare_to_curated/):                         ║
║    - prostate_obs_review.tsv                                   ║
║    - prostate_meds_review.tsv                                  ║
...
CONFIG = {
    ...
    "scores_dir": os.path.join(_SCRIPT_DIR, "Output", "Scores_prostate"),
    "curated_tsv": os.path.join(
        _SCRIPT_DIR, "..", "codelists", "prostate_curated_codes_v2.tsv"
    ),
    "output_dir": os.path.join(_SCRIPT_DIR, "Output", "Compare_to_curated"),
    "obs_all_csv": "prostate_obs_all.csv",
    "meds_all_csv": "prostate_meds_all.csv",
...
def load_ranked(csv_path, feature_type):
    if not os.path.exists(csv_path):
        raise FileNotFoundError(
            ...
            "  → Run Ranking.py first to produce prostate_*_all.csv files."
        )
