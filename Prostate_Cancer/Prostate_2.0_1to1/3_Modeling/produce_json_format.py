...
We can then load via _load_alex_features.py to verify modeling still works.
...
Outputs in --out-dir:
    prostate_1mo_jsonformat.csv       # post-ADR style CSV
    prostate_1mo_jsonformat.parquet   # post-ADR style parquet (recommended)
    prostate_1mo_schema.json          # feature schema sidecar
    prostate_1mo_conversion_report.md # before/after comparison
...
## Validation
Run `_load_alex_features.py` against the JSON-column file and confirm the
...
if __name__ == "__main__":
    ...
    ap.add_argument("--cancer", default="prostate", help="Cancer name (default: prostate)")
