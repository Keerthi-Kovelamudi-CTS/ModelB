...
def main(arg, target_sens=0.90):
    ...
    # APPLY the xpoll percentile reference fit during training (leakage-safe: held-out labs ranked
    # against the TRAINING distribution, not their own).
    ho_feat = lb.build_matrix(ho_ev, L, os.path.join(od, "heldout_features.csv"), full_ho,
                              xpoll_fit=False, xpoll_ref_path=os.path.join(od, f"xpoll_ref_{tag}.json"))
