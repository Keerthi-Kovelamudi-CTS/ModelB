...
class LungCancerPredictor:
    ...
    def hyperparameter_tuning(self, model_name=None):
        ...
        # Cap the inner estimator to ONE thread so the outer RandomizedSearchCV (n_jobs=-1) is the
        # only parallelism — otherwise 48 search workers x 48 model threads = ~2300 threads on 48
        # cores (load ~2000), which thrashes and runs far slower. 48 single-threaded fits = 48 threads.
