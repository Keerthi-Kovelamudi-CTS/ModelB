...
def patient_fixes(clin_df, med_df, window_name, cfg):
    ...
    if clin_df is not None:
        ...
        # Bladder: keep both sexes — cohort is already sex-stratified upstream in
        # Truveta/Bladder_Cancer/bladder_truveta.sql (~99% F, ~1% M cases included).
        # No filtering here; previous version dropped ~88 male bladder-cancer patients.
        if False and cfg.CANCER_NAME == 'bladder':
            non_female = clin_df[clin_df['SEX'] != 'F']
            if len(non_female) > 0:
                n_patients = non_female['PATIENT_GUID'].nunique()
                logger.warning(f"  Removing {n_patients} non-female patients (bladder cancer)")
