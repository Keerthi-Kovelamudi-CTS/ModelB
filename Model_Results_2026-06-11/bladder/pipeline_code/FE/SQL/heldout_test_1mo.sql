/*
 * Unified cancer / non-cancer patient events for ML training — BLADDER variant.
 *
 * This is a fixed-cohort variant of unified_cancer_noncancer42_v3.sql:
 *   - Cancer patients      = the 600 ids in  prj-cts-ai-dev-sp.EMIS_BULK_DATA_temp.bladder_cancer_600
 *   - Non-cancer patients  = the 50,000 ids in prj-cts-ai-dev-sp.EMIS_BULK_DATA_temp.no_cancer_bladder_50000
 *
...
WITH params AS (
  SELECT
    ...
    'bladder%'          AS target_cancer_pattern        -- target cancer (must match bladder_cancer_600)
),
...
/* Non-cancer anchor = one deterministically-sampled in-window observation event
   (identical to the logic that produced no_cancer_bladder_50000). */
...
cancer_selected AS (
  ...
  WHERE tcp.patient_guid IN (
    SELECT patient_guid FROM `prj-cts-ai-dev-sp.EMIS_BULK_DATA_temp.bladder_cancer_600`
  )
),
...
non_cancer_selected AS (
  ...
  WHERE nca.patient_guid IN (
    SELECT patient_guid FROM `prj-cts-ai-dev-sp.EMIS_BULK_DATA_temp.no_cancer_bladder_50000`
  )
),
