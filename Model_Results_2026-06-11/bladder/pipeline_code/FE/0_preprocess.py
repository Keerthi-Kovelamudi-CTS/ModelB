...
Reads:  data/raw/bladder_{window}_raw.csv   (output of SQL Queries/v4/{window}_v4.sql)
Writes: data/{window}/bladder_{window}.csv  (single unified file — obs + med UNION'd)
...
DEFAULT_RAW = SCRIPT_DIR / "data" / "raw" / "bladder_1mo_raw.csv"
...
def build_placeholder_rows(unified_patients, sex_lookup, age_lookup,
    ...
    for pid, anchor_date, label in unified_patients.itertuples(index=False):
        ...
        rows.append({
            "PATIENT_GUID": pid,
            "SEX": sex_lookup.get(pid),  # both sexes; None when missing
...
def main():
    ...
    ap.add_argument("--prefix",  default="bladder_1mo",            help="output filename prefix")
    args = ap.parse_args()
    ...
    # Parse digit-strings straight to Int64 — DMD med codes are 16-17 digits (>2^53), so routing them
    # through float64 (pd.to_numeric) loses precision and the safe Int64 cast then rejects them. int64
    # holds up to ~9.2e18, so a direct string->Int64 conversion is exact; non-numeric -> <NA>.
    _cid = (df["CODE_ID"].astype("string").str.strip()
            .str.replace(r"\.0$", "", regex=True))
    df["CODE_ID"] = _cid.where(_cid.str.fullmatch(r"\d+")).astype("Int64")
    ...
    # Date columns: real rows carry datetime, placeholder rows carry "%Y-%m-%d" strings -> the column
    # is mixed object and pyarrow can't write it. Normalise to a uniform YYYY-MM-DD string (NaT -> NA).
    for c in ('INDEX_DATE', 'EVENT_DATE'):
        if c in df_categorized.columns:
            df_categorized[c] = pd.to_datetime(df_categorized[c], errors='coerce').dt.strftime('%Y-%m-%d')
