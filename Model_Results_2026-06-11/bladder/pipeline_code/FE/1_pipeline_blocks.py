...
def build_clinical_features(clin_df, cfg, window_name):
    ...
    # Apply mappings (whether loaded or freshly fitted). For Bladder/Ovarian
    ...
    # — captures chronicity vs acute presentation. Computed over A∪B events for the patient.
    obs_AB_dated = obs_df[obs_df['TIME_WINDOW'].isin(['A', 'B'])].copy()
    ...
    for cat in lab_categories:
        ...
        # Percent change A→B. Dimensionless; comparable across patients regardless of
        # absolute level (HbA1c 5.5→6.5 vs 50→60 mmol/mol — same delta, very different clinical meaning).
        pct_change = (mean_B.subtract(mean_A)
                      / mean_A.replace(0, np.nan).abs()).replace([np.inf, -np.inf], np.nan)
        pct_change.name = f"LAB_{cat}_pct_change"
        ...
        parts = [stats, last_val, first_val, mean_A, mean_B, delta, pct_change,
                 slope, trend_r, has_ever, is_missing]
    ...
    for feat_df in [obs_A, obs_B, obs_total, obs_has_ever, obs_accel, obs_new_B, obs_temporal]:
...
def build_advanced_features(clin_df, med_df, existing_fm, window_name, cfg):
    ...
    if 'MONTHS_BEFORE_INDEX' in obs_windowed.columns:
        # Open-ended top bin: with a multi-year lookback, events older than 36mo must NOT be
        # silently dropped to NaN (that discarded most of the history) — they land in Q7_36m_plus.
        obs_windowed['QUARTER'] = pd.cut(
            obs_windowed['MONTHS_BEFORE_INDEX'],
            bins=[0, 6, 12, 18, 24, 30, 36, np.inf],
            labels=['Q1_0-6m', 'Q2_6-12m', 'Q3_12-18m', 'Q4_18-24m', 'Q5_24-30m', 'Q6_30-36m', 'Q7_36m_plus'],
            right=True
        )
...
def build_acceleration_features(clin_df, med_df, existing_fm, window_name, cfg):
    ...
    Generic lab recent-vs-lifetime velocity comparison across cfg.LAB_CATEGORIES.
    ...
    # 6. Lab recent-vs-lifetime velocity — generic across config.LAB_CATEGORIES.
    # Captures "this lab is accelerating recently relative to its lifetime trend".
    # For bladder, the meaningful labs are BMI + HbA1c (no single-marker analog
    # of PSA exists, so we apply the same logic across all configured labs).
    def _velocity_recent_vs_lifetime(group):
        if len(group) < 2:
            return pd.Series({'recent_v': np.nan, 'life_v': np.nan})
        g = group.sort_values('EVENT_DATE')
        g_recent = g[g['TIME_WINDOW'] == 'B']
    ...
    for lab_cat in getattr(cfg, 'LAB_CATEGORIES', []):
        lab_df = clin[(clin['CATEGORY'] == lab_cat) &
                       pd.to_numeric(clin['VALUE'], errors='coerce').notna()].copy()
        if len(lab_df) == 0:
            continue
        lab_df['VALUE'] = pd.to_numeric(lab_df['VALUE'], errors='coerce')
        vels = lab_df.groupby('PATIENT_GUID').apply(_velocity_recent_vs_lifetime)
        if vels.empty:
            continue
        af[f'ACCEL_{lab_cat}_recent_velocity_per_year']   = vels['recent_v'].reindex(af.index, fill_value=0).astype(float)
        af[f'ACCEL_{lab_cat}_lifetime_velocity_per_year'] = vels['life_v'].reindex(af.index, fill_value=0).astype(float)
        denom = af[f'ACCEL_{lab_cat}_lifetime_velocity_per_year'].abs().clip(lower=0.05)
        af[f'ACCEL_{lab_cat}_velocity_acceleration'] = (
            af[f'ACCEL_{lab_cat}_recent_velocity_per_year'] / denom
        ).clip(-50, 50)
