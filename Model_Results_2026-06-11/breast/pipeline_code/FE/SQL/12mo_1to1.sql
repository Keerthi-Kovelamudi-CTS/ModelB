/*
 * Unified cancer / non-cancer patient events for ML training — BREAST TRAINING.
 *
 * HOLD-OUT EXCLUSION: patients in breast_cancer_2000 and no_cancer_breast_50000 (the real-world
...
WITH params AS (
  SELECT
    ...
    '%breast%'          AS target_cancer_pattern        --      Specify the target cancer
...
holdout_patients AS (
  SELECT patient_guid FROM `prj-cts-ai-dev-sp.EMIS_BULK_DATA_temp.breast_cancer_2000`
  UNION DISTINCT
  SELECT patient_guid FROM `prj-cts-ai-dev-sp.EMIS_BULK_DATA_temp.no_cancer_breast_50000`
),
...
/* Sex-matched control sampling: match the controls' sex distribution to the
   cases' WITHIN each anchor-year, so both arms carry the same ~99%-female /
   ~1%-male mix. This removes the trivial male->control shortcut WITHOUT
   dropping the small male-case cohort, so the model still sees and can score
   both sexes. Sex is normalised to female/male/other. */
cancer_year_counts AS (
  SELECT
    EXTRACT(YEAR FROM cec.anchor_date) AS anchor_year,
    CASE
      WHEN LOWER(TRIM(CAST(p.sex AS STRING))) IN ('f','female') THEN 'female'
      WHEN LOWER(TRIM(CAST(p.sex AS STRING))) IN ('m','male')   THEN 'male'
      ELSE 'other'
    END                                AS sex_norm,
    COUNT(*)                           AS cancer_count
  FROM cancer_event_counts cec
  JOIN patients p USING (patient_guid)
  GROUP BY 1, 2
),
...
non_cancer_ranked AS (
  SELECT
    ...
    ROW_NUMBER() OVER (
      PARTITION BY
        EXTRACT(YEAR FROM ncec.anchor_date),
        CASE
          WHEN LOWER(TRIM(CAST(p.sex AS STRING))) IN ('f','female') THEN 'female'
          WHEN LOWER(TRIM(CAST(p.sex AS STRING))) IN ('m','male')   THEN 'male'
          ELSE 'other'
        END
      ORDER BY FARM_FINGERPRINT(CONCAT(CAST(ncec.patient_guid AS STRING), '|ncsalt'))
    ) AS rank_in_year
...
non_cancer_sampled AS (
  ...
  JOIN cancer_year_counts cyc
    ON cyc.anchor_year = ncr.anchor_year
   AND cyc.sex_norm   = ncr.sex_norm
  CROSS JOIN params param
