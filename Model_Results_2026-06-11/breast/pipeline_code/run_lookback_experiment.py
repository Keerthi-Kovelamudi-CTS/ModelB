"""
Breast HYBRID lookback experiment: the MATURE breast FE engine (8-phase, ~1-2k rich
...
GCS = "gs://cancer_transformed_ai/breast_lookback"
...
metrics = _load(os.path.join(MODEL_DIR, "breast_metrics.py"), "breast_metrics")
...
import config as breast_cfg            # FE/config.py  (CANCER_NAME/DATA_PREFIX='breast')
...
import breast_extra_features as extra   # problem-list flags + NLR/PLR (post-FE enrichment from raw events)
...
def ensure_events():
    ...
    if os.path.exists(TRAIN_EVENTS):
        ...
        rp.export_query_to_csv(open(os.path.join(FE_DIR, "SQL", f"{GAP}mo_1to{NC_RATIO}.sql")).read(),
                               TRAIN_EVENTS, gcs_prefix=GCS, tag=f"breast_train_lifetime_{GAP}mo_r{NC_RATIO}")
    if os.path.exists(HELDOUT_EVENTS):
        ...
        rp.export_query_to_csv(open(os.path.join(FE_DIR, "SQL", f"heldout_test_{GAP}mo.sql")).read(),
                               HELDOUT_EVENTS, gcs_prefix=GCS, tag=f"breast_heldout_lifetime_{GAP}mo")
...
def _set_cfg_paths(work):
    ...
    work = Path(work)
    breast_cfg.BASE_PATH       = work / "data"
    breast_cfg._RESULTS_ROOT   = work / "results"
    breast_cfg.FE_RESULTS      = work / "results" / "3_feature_engineering"
    breast_cfg.SANITY_RESULTS  = work / "results" / "1_sanity_check"
    breast_cfg.CLEANUP_RESULTS = work / "results" / "5_cleanup"
    breast_cfg.WINDOWS         = ["12mo"]
    for p in (breast_cfg.BASE_PATH / "12mo", breast_cfg.BASE_PATH / "raw",
              breast_cfg.FE_RESULTS, breast_cfg.SANITY_RESULTS, breast_cfg.CLEANUP_RESULTS):
...
def run_mature_fe(events_df, L, work):
    ...
    raw_csv = str(Path(breast_cfg.BASE_PATH) / "raw" / "breast_12mo_raw.csv")
    df.to_csv(raw_csv, index=False)
    out_dir = str(Path(breast_cfg.BASE_PATH) / "12mo")

    # 0_preprocess.py is argparse-only -> drive via subprocess; writes {out_dir}/breast_12mo.parquet
    r = subprocess.run([sys.executable, os.path.join(FE_DIR, "0_preprocess.py"),
                        "--raw", raw_csv, "--out-dir", out_dir,
                        "--mapping", CODELIST, "--prefix", "breast_12mo"],
                       capture_output=True, text=True)
    assert r.returncode == 0, f"0_preprocess failed:\n{r.stderr[-1800:]}"

    sanity_mod.run_sanity_check(breast_cfg)      # split obs/med, dedup, drops
    clean_mod.run_clean_data(breast_cfg)         # leak guard
    fe_pipeline.run_feature_engineering(breast_cfg)   # 8-phase FE

    fm_path = Path(breast_cfg.FE_RESULTS) / "12mo" / "feature_matrix_final_12mo.parquet"
...
def main():
    print("=" * 70 + "\nBREAST HYBRID LOOKBACK SWEEP (5/10/20/lifetime) — mature FE + improved modeling\n" + "=" * 70)
    tr, _ = ensure_events()
