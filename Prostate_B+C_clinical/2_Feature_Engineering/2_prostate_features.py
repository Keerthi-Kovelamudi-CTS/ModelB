...
def build_cancer_specific_features(clin_df, med_df, existing_fm, window_name, cfg=None):
    ...
    obs_AB = clin[clin['TIME_WINDOW'].isin(['acute', 'subacute', 'recent_past', 'historical'])].copy()
    obs_A = clin[clin['TIME_WINDOW'] == 'subacute'].copy()
    obs_B = clin[clin['TIME_WINDOW'] == 'acute'].copy()
    med_AB = med[med['TIME_WINDOW'].isin(['acute', 'subacute', 'recent_past', 'historical'])].copy()
    med_A = med[med['TIME_WINDOW'] == 'subacute'].copy()
    med_B = med[med['TIME_WINDOW'] == 'acute'].copy()
    ...
    try:
        ...
        psa_A = psa_all[psa_all['TIME_WINDOW'] == 'subacute'].groupby('PATIENT_GUID')['VALUE'].mean()
        psa_B = psa_all[psa_all['TIME_WINDOW'] == 'acute'].groupby('PATIENT_GUID')['VALUE'].mean()
    ...
    pf[f'{PREFIX}urinary_count_subacute'] = urinary_A
    pf[f'{PREFIX}urinary_count_acute'] = urinary_B
    pf[f'{PREFIX}urinary_accel_acute_vs_subacute'] = (urinary_B > urinary_A).astype(int)

    pf[f'{PREFIX}has_haematuria'] = has_cat(obs_AB, 'HAEMATURIA')
    pf[f'{PREFIX}haematuria_count_acute'] = count_cat(obs_B, 'HAEMATURIA')
    ...
    pf[f'{PREFIX}bone_count_subacute'] = m(bone_A)
    pf[f'{PREFIX}bone_count_acute'] = m(bone_B)
    pf[f'{PREFIX}bone_accel_acute_vs_subacute'] = (m(bone_B) > m(bone_A)).astype(int)
    ...
    pf[f'{PREFIX}uti_abx_count_subacute'] = count_cat(med_A, 'UTI_ANTIBIOTICS')
    pf[f'{PREFIX}uti_abx_count_acute'] = count_cat(med_B, 'UTI_ANTIBIOTICS')
    pf[f'{PREFIX}pain_med_count_acute'] = count_cat(med_B, 'PAIN_ESCALATION')
    ...
    pf[f'{PREFIX}AGEX_LUTS_count_acute']         = age * luts_B
    pf[f'{PREFIX}AGEX_HAEMATURIA_count_acute']   = age * haem_B
    pf[f'{PREFIX}AGEX_PAIN_count_acute']         = age * pain_B
    pf[f'{PREFIX}AGEX_BONE_count_acute']         = age * bone_B_c
    ...
    pf[f'{PREFIX}AGEX_LUTS_accel_acute_vs_subacute'] = age * (luts_B - luts_A).clip(lower=0)
    pf[f'{PREFIX}AGEX_BONE_accel_acute_vs_subacute'] = age * ((pain_B + bone_B_c) - (pain_A + bone_A_c)).clip(lower=0)
    ...
    pf[f'{PREFIX}AGEX_PAIN_MED_B']      = age * _safe(f'{PREFIX}pain_med_count_acute')
    pf[f'{PREFIX}AGEX_UTI_ABX_B']       = age * _safe(f'{PREFIX}uti_abx_count_acute')
    ...
    pf[f'{PREFIX}ACTIVITY_accel_acute_vs_subacute']   = (events_B / events_A.replace(0, 1)).clip(upper=10)  # max 10x to avoid inf
