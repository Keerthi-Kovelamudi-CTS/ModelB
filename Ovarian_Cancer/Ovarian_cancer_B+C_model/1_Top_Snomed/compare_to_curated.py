"""
╔══════════════════════════════════════════════════════════════════╗
║  OVARIAN — Phase 1.4 (Clinical Review Bridge)                ║
║  Compare Ranking.py output vs. existing curated codelist.     ║
║                                                                ║
║  Inputs:                                                       ║
║    - Output/Scores_ovarian/ovarian_obs_all.csv               ║
║    - Output/Scores_ovarian/ovarian_meds_all.csv              ║
║    - ../codelists/ovarian_curated_codes_v2.tsv                ║
║                                                                ║
║  Outputs (Output/Compare_to_curated/):                         ║
║    - ovarian_obs_review.tsv                                   ║
║    - ovarian_meds_review.tsv                                  ║
║    - ovarian_combined_review.tsv                              ║
...
CONFIG = {
    "cancer_type": "ovarian",
    "scores_dir": os.path.join(_SCRIPT_DIR, "Output", "Scores_ovarian"),
    "curated_tsv": os.path.join(
        _SCRIPT_DIR, "..", "codelists", "ovarian_curated_codes_v2.tsv"
    ),
    "output_dir": os.path.join(_SCRIPT_DIR, "Output", "Compare_to_curated"),
    "obs_all_csv": "ovarian_obs_all.csv",
    "meds_all_csv": "ovarian_meds_all.csv",
...
def load_ranked(csv_path, feature_type):
    if not os.path.exists(csv_path):
        raise FileNotFoundError(
            ...
            "  → Run Ranking.py first to produce ovarian_*_all.csv files."
        )
