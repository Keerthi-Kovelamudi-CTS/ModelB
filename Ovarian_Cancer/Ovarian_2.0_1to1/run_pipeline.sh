#!/bin/bash
# Ovarian_2.0_1to1 end-to-end pipeline (Approach B)
#
# Steps:
#   1. SQL extract  → data/raw/ovarian_1mo_raw.csv  (run separately in BQ Studio or via bq client)
#   2. Preprocess   → data/1mo/{ovarian_1mo_obs_dropped, ovarian_1mo_med_dropped}.csv
...
run_preprocess () {
  ...
  for w in "${windows[@]}"; do
    ...
    python3 0_preprocess_to_fe.py \
      --raw     data/raw/ovarian_${w}_raw.csv \
      --out-dir data/${w} \
      --prefix  ovarian_${w}
  done
}
...
run_eval () {
  ...
  if [ ! -s "data/fe_input/${WINDOW}/ovarian_${WINDOW}_obs_dropped.csv" ]; then
