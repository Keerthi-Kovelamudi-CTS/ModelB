# ═══════════════════════════════════════════════════════════════
# OVARIAN CANCER — CLINICAL + TEXT + EMBEDDINGS (4 SEEDS)
...
RESULTS_PATH = SCRIPT_DIR / 'text_emb_4seeds_results'
...
def is_clinical_feature(col):
    ...
    # KEEP: NICE guideline features
    if col.startswith('NICE_'):
        return True

    # KEEP: Ovarian-specific clinical features
    if col.startswith('OV_'):
        return True
    ...
    # KEEP: Comorbidity flags and interactions (specific conditions)
    if col.startswith('COMORB_') or col.startswith('COMORBX_'):
        return True
    ...
    if col.startswith('INV_PATTERN_'):
        return True
    if col.startswith('INV_') and 'gynae' in col.lower():
        return True
    ...
    # KEEP: Trajectory features for symptoms/pain/GI (clinically meaningful)
    if col.startswith('TRAJ_'):
        # Keep pain and GI trajectories, increasing trend
        if 'pain' in col.lower() or 'gi' in col.lower() or 'trend' in col.lower():
            return True
