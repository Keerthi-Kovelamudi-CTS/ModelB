# ═══════════════════════════════════════════════════════════════
# OVARIAN CANCER — BALANCED MODELING (UNDERSAMPLING)
# Same as 1_run_modeling but undersamples negatives in train
# so train has neg ≈ pos (1:1 ratio)
# ═══════════════════════════════════════════════════════════════
...
VAL_RATIO = 0.10
TEST_RATIO = 0.15
SEEDS = [42, 43, 44, 45]  # 4 seeds for stable results
...
RESULTS_PATH = SCRIPT_DIR / 'balanced_results'
...
def undersample_train(X, y, seed, neg_to_pos_ratio=1.0):
    """Keep all positives; randomly sample negatives so n_neg ≈ n_pos * neg_to_pos_ratio."""
    ...
    n_pos = len(pos_idx)
    n_neg = len(neg_idx)
    n_neg_keep = min(n_neg, max(1, int(n_pos * neg_to_pos_ratio)))
    neg_perm = rng.permutation(n_neg)
    neg_keep = neg_idx[neg_perm[:n_neg_keep]]
    keep_idx = np.concatenate([pos_idx, neg_keep])
    rng.shuffle(keep_idx)
    if isinstance(X, pd.DataFrame):
        return X.iloc[keep_idx], y.iloc[keep_idx]
    return X[keep_idx], y.iloc[keep_idx] if hasattr(y, 'iloc') else y[keep_idx]
...
for seed in SEEDS:
    for window in ['3mo', '6mo', '12mo']:
        ...
        # Undersample negatives in train to balance classes
        X_train_res, y_train_res = undersample_train(X_train_sel, y_train, seed=seed, neg_to_pos_ratio=1.0)
        print(f"  Balanced train: {X_train_res.shape[0]} ({(y_train_res==1).sum()} pos, {(y_train_res==0).sum()} neg)")
        train_spw = spw
        ...
        # 4. Use FULL val set for threshold search (no cal/thresh split, no calibration)
        ...
        # 6. Threshold search on FULL VAL; report on TEST
        ...
        for model_name, pred_test in predictions.items():
            ...
            # Forced operating points: 80/70 and 75/65
        ...
        # Operating point table — first seed only
        if seed == SEEDS[0]:
            print(f"\n  ── Operating points (val) ──")
