# ═══════════════════════════════════════════════════════════════
# OVARIAN CANCER — PREDICT ON UNSEEN DATA
...
def predict(models, config, feature_matrix):
    ...
    # ── CONFIDENCE SCORE ──
    # Count how many non-zero features each patient has (among model features)
    # Patients with very few approved code events → unreliable predictions
    non_zero_counts = (X != 0).sum(axis=1)
    # Exclude demographics (AGE_AT_INDEX, AGE_BAND) from count — everyone has age
    ...
    print(f"  LOW = <5 non-zero features (insufficient data — prediction unreliable)")
    print(f"  MEDIUM = 5-14 features | HIGH = 15+ features")

    for name, model in models.items():
        proba = model.predict_proba(X)[:, 1]
        predictions[f'{name}_prob'] = proba
        threshold = config.get(f'threshold_{name}', 0.5)
        # Only flag HIGH/MEDIUM confidence patients; LOW confidence → always 0 (not enough data)
        predictions[f'{name}_flag'] = ((proba >= threshold) & (predictions['confidence'].values != 'LOW')).astype(int)
    ...
    if top2[0] in models and top2[1] in models:
        ...
        predictions['Ensemble_flag'] = ((ens_prob >= ens_threshold) & (predictions['confidence'].values != 'LOW')).astype(int)
...
def main():
    ...
    print(f"{'='*60}")
    print(f"  OVARIAN CANCER — PREDICT ON UNSEEN DATA")
    ...
    # Run predictions
    print(f"\n  Running predictions...")
    predictions = predict(models, config, fm)
