...
# Output: same prints to console and to results/sanity_check/sanity_check_ovarian.log
SCRIPT_DIR = Path(__file__).resolve().parent
OUTPUT_DIR = SCRIPT_DIR / 'results' / 'sanity_check'
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
OUTPUT_FILE = OUTPUT_DIR / 'sanity_check_ovarian.log'
...
files = {
    '3mo': {
        'clinical': f"{BASE_PATH}/3mo/FE_ovarian_clinical_windowed_3m.csv",
        'med':      f"{BASE_PATH}/3mo/FE_ovarian_med_windowed_3m.csv",
    },
    '6mo': {
        'clinical': f"{BASE_PATH}/6mo/FE_ovarian_clinical_windowed_6m.csv",
        'med':      f"{BASE_PATH}/6mo/FE_ovarian_med_windowed_6m.csv",
    },
    '12mo': {
        'clinical': f"{BASE_PATH}/12mo/FE_ovarian_clinical_windowed_12m.csv",
        'med':      f"{BASE_PATH}/12mo/FE_ovarian_med_windowed_12m.csv",
    },
}
...
try:
    print(f"Sanity check + Patient overlap check — Ovarian pipeline")
    ...
    def patient_fixes(clin_df, med_df, window_name):
        ...
        if clin_df is not None:
            ...
            clin_df = clin_df.drop_duplicates(
                subset=['PATIENT_GUID', 'EVENT_DATE', 'SNOMED_ID', 'CATEGORY', 'LABEL']
            )
            after = len(clin_df)
        ...
        if clin_df is not None:
            ...
            print(f"  {sex_dist.to_string()}")
            if clin_df['SEX'].nunique() > 1 or (clin_df['SEX'].nunique() == 1 and clin_df['SEX'].iloc[0] != 'F'):
                print(f"  ⚠️ Non-female patients found!")
                non_f = clin_df[clin_df['SEX'] != 'F']
                print(f"  Removing {non_f['PATIENT_GUID'].nunique()} non-female patients...")
                clin_df = clin_df[clin_df['SEX'] == 'F']
            else:
                print(f"  ✅ All female")
            ...
            if under_18 > 0:
                ...
                clin_df = clin_df[clin_df['AGE_AT_INDEX'] >= 18]
            else:
                print(f"  ✅ All 18+")
    ...
    for window in ['3mo', '6mo', '12mo']:
        ...
        if clin is not None:
            clin_path = f"{BASE_PATH}/{window}/FE_ovarian_dropped_patients_clinical_windowed_{suffix}.csv"
            clin.to_csv(clin_path, index=False)
            print(f"\n  ✅ Saved: {clin_path} ({len(clin):,} rows)")

        if med is not None:
            med_path = f"{BASE_PATH}/{window}/FE_ovarian_dropped_patients_med_windowed_{suffix}.csv"
