# ═══════════════════════════════════════════════════════════════
# OVARIAN CANCER — FEATURE CLEANUP
...
def cleanup_features(fm, window_name):
    ...
    labcount_cols = [c for c in fm.columns if c.startswith('LABTERM_') and c.endswith('_count')]
    utilization_confounds = []
    for col in labcount_cols:
    ...
    for i, (col, corr) in enumerate(label_corr_final.head(30).items()):
        is_ov = col.startswith('NICE_') or col.startswith('OV_')
        is_adv = any(col.startswith(p) for p in [
            ...
            'RECUR_','MEDREC_','RATE_','AGE_','AGEX_','COMORBX_',
            'ENTROPY_','GINI_','COMORB_burden','COMORB_high'
        ])
        if is_ov:
            marker = '◆'
        elif is_adv:
    ...
    print(f"\n  ◆ = ovarian-specific (Step 4d) | ★ = advanced/mega (Step 4b/4c)")
    ...
    print(f"\n  Feature group breakdown:")
    prefixes = [
        ('NICE_', 'NICE guidelines'),
        ('OV_', 'Ovarian-specific'),
        ('OBS_', 'Observation'),
...
for window in ['3mo', '6mo', '12mo']:
    ...
    print(f"    Ovarian-specific (◆): {len(ov_cols)}")
...
# ═══════════════════════════════════════════════════════════════
# OVARIAN-SPECIFIC SIGNAL CHECK
# ═══════════════════════════════════════════════════════════════

print(f"\n{'═'*70}")
print(f"  OVARIAN-SPECIFIC FEATURES — SIGNAL RANK")
print(f"{'═'*70}")

for window in ['3mo', '6mo', '12mo']:
    fm = cleaned_matrices[window]
    ov_cols = [c for c in fm.columns if (c.startswith('NICE_') or c.startswith('OV_')) and c != 'LABEL']
    
    if ov_cols:
        ov_corr = fm[ov_cols].corrwith(fm['LABEL']).abs().sort_values(ascending=False)
        print(f"\n  {window} — Ovarian features ranked:")
        for i, (col, corr) in enumerate(ov_corr.items()):
