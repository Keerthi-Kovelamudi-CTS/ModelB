...
/* Per-cancer-type configuration derived from params.cancer_cohort. */
...
/* ────────────────────────────────────────────────────────────────────────
   FIX 1: Restrict the patient universe to the target states UP FRONT.
   ──────────────────────────────────────────────────────────────────────── */
eligible_patients AS (
  SELECT p.person_id AS patient_id, p.cohort
...
/* Random anchor for non-cancer patients drawn from the cancer-cohort
   diagnosis-date distribution. Matches v3's temporal-balancing approach. */
...
observation_events_raw AS (
  ...
  WHERE mh.event_date >= DATE_SUB(up.anchor_date, INTERVAL p.years_before  YEAR)
    ...
    -- state filter no longer needed here — medical_history is already restricted via eligible_patients (FIX 1)
),
...
medication_events_raw AS (
  ...
  WHERE mh.event_date >= DATE_SUB(up.anchor_date, INTERVAL p.years_before  YEAR)
    AND mh.event_date <  DATE_SUB(up.anchor_date, INTERVAL p.months_before MONTH)
    -- state filter no longer needed here — medication_history is already restricted via eligible_patients (FIX 1)
),
