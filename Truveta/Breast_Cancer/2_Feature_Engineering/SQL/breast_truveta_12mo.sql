...
WITH params AS (
  SELECT
    ...
    5    AS non_cancer_ratio,                                                 -- oversample non-cancer 5x as a buffer; build_shared_split downsamples to 1:1 across train/val/test so positives are never discarded
  ...
  WHERE cohort = (SELECT cancer_cohort FROM params)
    ...
    AND (
      ...
      OR (vocabulary = 'snomed' AND (
        -- Family history exception: don't exclude FH-of-cancer terms (real risk factor, not patient diagnosis)
        LOWER(display) NOT LIKE '%family history%' AND
        LOWER(display) NOT LIKE 'fh:%' AND
        LOWER(display) NOT LIKE 'family hx%'
      ) AND (
        ...
        -- LEAKAGE FIX round-3 (2026-06-01): SURGICAL — abnormal mammography findings only
        LOWER(display) LIKE '%mammography abnormal%' OR
        LOWER(display) LIKE '%abnormal%mammogr%' OR
        LOWER(display) LIKE '%mammograph%assessment%category%' OR    -- BIRADS Cat 0,3,4,5,6
        LOWER(display) LIKE '%inconclusive%mammogr%' OR
        LOWER(display) LIKE '%mammographic microcalcification%' OR
        LOWER(display) LIKE '%abnormal findings%diagnostic imaging%breast%' OR
        LOWER(display) LIKE '%ultrasonography of breast abnormal%' OR
        LOWER(display) LIKE '%acquired absence%breast%' OR    -- post-mastectomy marker
        LOWER(display) LIKE '%history of breast reconstruction%' OR
        LOWER(display) LIKE '%contour of breast distorted%' OR
      ...
      OR (vocabulary IN ('cpt','hcpcs') AND (
        ...
        -- LEAKAGE FIX round-3 (2026-06-01): SURGICAL — diagnostic mammography ONLY (not screening)
        LOWER(display) LIKE '%diagnostic mammog%' OR
        LOWER(display) LIKE '%diagnostic digital breast tomosynthesis%' OR
        LOWER(display) LIKE '%screening mammogram and diagnostic%' OR    -- combined pmt
        LOWER(display) LIKE 'mri%breast%' OR LOWER(display) LIKE '%magnetic resonance imaging, breast%' OR
        LOWER(display) LIKE 'ultrasound%breast%' OR LOWER(display) LIKE '%ultrasound, breast%' OR
        LOWER(display) LIKE 'mri%mammary%' OR
        LOWER(display) LIKE '%computer-aided detection%mammography%with further review%' OR
...
-- ── Gold-layer anchor (aligned to cancer_training_truveta_v3_gold.sql) ──────
-- Non-cancer patients are anchored on a RANDOM date drawn from the CANCER
-- cohort's diagnosis-date distribution (cancer_anchor_pool) — NOT their own
-- event date. This guarantees by construction that non-cancer and cancer share
-- the same anchor-YEAR distribution (the v3 "distribution match guaranteed"
-- approach), which is more exact than per-year stratification on self-anchored
-- dates. Assignment is deterministic via FARM_FINGERPRINT(patient_id).
...
-- Sex + ANCHOR-YEAR distribution of the cancer cohort. Used to stratify the
-- non-cancer sample so the two cohorts share BOTH the same sex mix AND the same
-- anchor-year distribution.
--
-- WHY (SEX, YEAR)-STRAT on top of the gold-layer anchor:
--   The gold anchor (cancer_anchor_pool above) already matches the anchor-YEAR
--   distribution by construction. This sampler additionally matches SEX — which
--   the gold/lung template does NOT do, but breast needs: breast cancer is ~99%
--   female, so without sex-matching the non-cancer pool (both sexes) would let
--   the model learn "male → non-cancer". Matching per (sex, anchor_year) keeps
--   both the sex mix AND the anchor-year mix aligned between the two cohorts.
cancer_sex_year_distribution AS (
  SELECT
    d.sex,
...
non_cancer_sampled AS (
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (
        PARTITION BY d.sex, EXTRACT(YEAR FROM ncec.anchor_date)
        ORDER BY FARM_FINGERPRINT(CAST(ncec.patient_id AS STRING))
      ) AS sample_rn_within_year_sex
    FROM non_cancer_event_counts ncec
    JOIN demographics d
      ON d.patient_id = ncec.patient_id
     AND d.cohort = 'non_cancer'
  ) ncs
  JOIN cancer_sex_year_distribution csyd
    ON csyd.sex = ncs.sex
   AND csyd.anchor_year = ncs.anchor_year
  CROSS JOIN params p
  WHERE ncs.sample_rn_within_year_sex <= p.non_cancer_ratio * csyd.cancer_count
),
