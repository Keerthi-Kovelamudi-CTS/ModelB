...
Usage:
    ...
    python predict_single_patient.py --window 12mo --features-json '{"patient_age": 67, ...}'
"""
...
def load_artifacts(window, results_root=None):
    """Load model config + per-seed base models + meta-learners.

    results_root: path to 3_Modeling/results. Defaults to PROJECT_ROOT/3_Modeling/results.
    """
...
def predict(feature_vector, window, results_root=None):
    """Predict cancer risk for one patient.

    feature_vector: pandas Series indexed by feature names, OR dict {name: value}.
    window: '1mo' or '12mo'.
    results_root: optional override for the model artifacts dir (see load_artifacts).
    ...
    """
    cfg, seed_artifacts = load_artifacts(window, results_root=results_root)
    ...
    # Determine age band
    age = float(s.get("patient_age", 65.0))
...
def main():
    ...
    g.add_argument("--features-csv", help="One-row CSV with feature columns (and patient_age)")
