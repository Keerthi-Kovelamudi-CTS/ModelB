"""
╔══════════════════════════════════════════════════════════════════╗
║  BREAST CANCER — Phase 1.3                                     ║
║  Combined Feature Ranking (Statistical + ML)                   ║
║                                                                ║
║  Input Files (from Data_Input/):                               ║
║    - Breast_positive_obs.csv, Breast_negative_obs.csv          ║
║    - Breast_positive_med.csv, Breast_negative_med.csv          ║
║                                                                ║
║  Output Files: output/Scores_breast/                           ║
║      - breast_obs_top150.csv                                   ║
║      - breast_meds_top150.csv                                  ║
║      - breast_combined_top150.csv       ← FINAL DELIVERABLE    ║
║      - breast_obs_all.csv                                      ║
║      - breast_meds_all.csv                                     ║
║      - breast_combined_all.csv                                 ║
...
CONFIG = {
    ...
    'data_dir': _DATA_DIR,
    'input_files': {
        'pos_obs':         'Breast_positive_obs.csv',
        'neg_obs':         'Breast_negative_obs.csv',
        'pos_meds':        'Breast_positive_med.csv',
        'neg_meds':        'Breast_negative_med.csv',
    },
    ...
    'output_dir': os.path.join(_SCRIPT_DIR, 'output', 'Scores_breast'),
    ...
    'p_value_test':        'fisher',  # 'fisher' (exact, recommended) or 'chi2' (faster, approx)
    'multiple_testing':    'fdr_bh', # 'fdr_bh' (Benjamini-Hochberg, recommended) or 'bonferroni'
    'max_p_corrected':     0.05,   # threshold against the chosen correction column
...
# Per-vocab id columns are dropped from the output; everything is unified into the
# `code_id` column with the actual vocabulary in `code_type` (Truveta is multi-vocab:
# loinc / snomed / cpt / icd10cm / cvx / rxnorm / ...).
REDUNDANT_ID_COLS = {
    'snomed_id', 'snomed_id_pos', 'snomed_id_neg',
    'med_code_id', 'med_code_id_pos', 'med_code_id_neg',
    'vocabulary', 'vocabulary_pos', 'vocabulary_neg',  # raw vocab cols superseded by code_type
}
PRIORITY_FRONT_COLS = ['code_id', 'code_type', 'term', 'feature_type']


def save_csv(df, filename, top_n=None):
    filepath = os.path.join(CONFIG['output_dir'], filename)
    output = df.head(top_n).copy() if top_n else df.copy()
    # Drop redundant per-vocab id / vocab cols (replaced by unified code_id + code_type)
    output = output.drop(columns=[c for c in REDUNDANT_ID_COLS if c in output.columns])
    # Move identity + label cols to the front for readability
    front = [c for c in PRIORITY_FRONT_COLS if c in output.columns]
...
def benjamini_hochberg(p_values):
    ...
    Less conservative than Bonferroni — controls expected false-discovery rate
    rather than family-wise error. Standard in biomarker discovery; lets through
    rare-but-strong signals (BRCA1, low-prevalence comorbidities) that Bonferroni
    would crush with ~10k multiple-testing penalty.

    Returns array of adjusted p-values clipped to [0, 1]. NaN inputs map to 1.0.
    """
    ...
    # Enforce monotonicity from the top (largest p) down — standard BH step.
...
def compute_statistical_ranking(pos_df, neg_df, feature_type='observation'):
    ...
    # Use per-code vocabulary from the input CSV if present (Truveta — multi-vocab:
    # loinc/snomed/cpt/icd10cm/cvx/rxnorm/...). Falls back to the EMIS-style hardcoded
    # label if the CSV doesn't have a vocabulary column.
    if 'vocabulary_pos' in merged.columns:
        merged['code_type'] = merged['vocabulary_pos'].fillna(merged.get('vocabulary_neg', pd.NA))
    elif 'vocabulary' in merged.columns:
        merged['code_type'] = merged['vocabulary']
    else:
    ...
    # Compute BOTH chi-squared and Fisher's exact for every 2x2 contingency.
    # Fisher is exact and handles small counts correctly (matters for rare codes
    # like BRCA1, n=29). Chi-squared is kept for ranking-by-effect (chi2_score)
    # and backward compatibility. The chosen p-value (config.p_value_test) is
    # what feeds the correction + filter.
    chi2_vals, p_chi2, p_fisher = [], [], []
    for _, row in merged.iterrows():
        ...
        try:
            chi2, pc, _, _ = chi2_contingency(table, correction=True)
            chi2_vals.append(round(chi2, 6))
            p_chi2.append(pc)
        except Exception:
            chi2_vals.append(np.nan); p_chi2.append(np.nan)
        try:
            _, pf = fisher_exact(table, alternative='two-sided')
            p_fisher.append(pf)
        except Exception:
            p_fisher.append(np.nan)

    merged['chi_squared']     = chi2_vals
    merged['p_value_chi2']    = p_chi2
    merged['p_value_fisher']  = p_fisher
    # `p_value` is the primary column used downstream — points at the chosen test.
    test_choice = CONFIG.get('p_value_test', 'fisher').lower()
    merged['p_value'] = merged['p_value_fisher'] if test_choice == 'fisher' else merged['p_value_chi2']
    ...
    # Compute BOTH Bonferroni and BH-FDR corrections BEFORE filtering.
    # Bonferroni is preserved as a column for transparency / sanity; FDR is the
    # default filter because it controls expected-false-discovery rate and is
    # less conservative — rare-but-strong signals (BRCA1, n=29) can pass.
    n_tests = max(len(merged), 1)
    merged['p_value_bonferroni'] = np.minimum(merged['p_value'] * n_tests, 1.0)
    merged['p_value_fdr']        = benjamini_hochberg(merged['p_value'].values)

    correction = CONFIG.get('multiple_testing', 'fdr_bh').lower()
    p_corrected_col = 'p_value_fdr' if correction == 'fdr_bh' else 'p_value_bonferroni'

    if CONFIG.get('case_enriched_only', True):
        before = len(merged)
        min_or  = CONFIG.get('min_odds_ratio', 1.0)
        max_p   = CONFIG.get('max_p_corrected', 1.0)
        min_n   = CONFIG.get('min_case_count', 1)

        mask = (
            np.isfinite(merged['odds_ratio'])
            & (merged['odds_ratio'] > min_or)
            & (merged[p_corrected_col] < max_p)
            & (merged['n_patient_count_pos'] >= min_n)
        )
        merged = merged[mask].copy()
        logger.info(
            f"  Significance + effect filter  "
            f"(test={CONFIG.get('p_value_test', 'fisher')}, "
            f"correction={correction}, OR>{min_or}, "
            f"{p_corrected_col}<{max_p}, n_pos>={min_n}): "
    ...
    logger.info(f"  Significant (raw p<0.05):  {(merged['p_value'] < 0.05).sum()}")
    logger.info(f"  Bonferroni sig (p<0.05):   {(merged['p_value_bonferroni'] < 0.05).sum()}")
    logger.info(f"  FDR-BH sig (p<0.05):       {(merged['p_value_fdr'] < 0.05).sum()}")
