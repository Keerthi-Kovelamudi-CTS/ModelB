/*╔══════════════════════════════════════════════════════════════════╗
  ...
  ║  Top Medications — POSITIVE COHORT                             ║
  ║                                                                 ║
  ║  Source:  prj-cts-ai-dev-sp.truveta_gold               ║
  ║           .breast_cohort_events  (event_type = 'medication')    ║
  ║                                                                 ║
  ║  Window:  [anchor - 15mo, anchor - 1mo)                         ║
  ║           Filtered inline via anchor_date column.               ║
  ║                                                                 ║
  ║  Output columns (match prostate Ranking.py contract):           ║
  ║    med_code_id, term, median_diagnosis_age, median_event_age,   ║
  ║    n_patient_count, n_patient_count_total, prevalence,          ║
  ║    total_prescriptions, avg_prescriptions_per_patient,          ║
  ║    avg_duration, median_duration, std_duration                  ║
  ║                                                                 ║
  ║  Notes:                                                         ║
  ║    - med_code_id holds Truveta med_concept_id (dedup prefers    ║
  ║      RxNorm; see breast_truveta.sql medication_history CTE).    ║
  ║    - duration = stop_date - start_date in days (may be NULL).   ║
  ╚══════════════════════════════════════════════════════════════════╝*/
