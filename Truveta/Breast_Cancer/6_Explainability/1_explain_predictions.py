...
CLEANUP_RESULTS = PROJECT_ROOT / "2_Feature_Engineering" / "data" / "features"
...
def load_window(window, seed):
    ...
    Returns age bands as labelled strings — derived from continuous patient_age if
    present, otherwise reconstructed from the binary AGE_over_50/65/75 flags that
    survive features. Band edges match the modeling script's: <55, 55-65, 65-75, 75+.
    """
    ...
    fm_path = CLEANUP_RESULTS / window / "breast_feature_matrix.parquet"
    if not fm_path.exists():
        fm_path = CLEANUP_RESULTS / window / "breast_feature_matrix.csv"
    if not fm_path.exists():
    ...
    if "patient_age" in fm.columns:
        ages_num = fm["patient_age"].values
